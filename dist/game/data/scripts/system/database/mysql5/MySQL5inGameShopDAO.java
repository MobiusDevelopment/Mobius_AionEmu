...
public class MySQL5inGameShopDAO extends InGameShopDAO
{
	...
	{
		final FastMap<Byte, List<IGItem>> items = FastMap.newInstance();
		Connection con = null;
		try
		{
			...
			while (rset.next())
			{
				final byte category = rset.getByte("category");
				final byte subCategory = rset.getByte("sub_category");
				if (subCategory < 3)
				{
					continue;
				}
				...
				final long itemPrice = rset.getLong("item_price");
				final int list = rset.getInt("list");
				final int salesRanking = rset.getInt("sales_ranking");
				final byte itemType = rset.getByte("item_type");
				final byte gift = rset.getByte("gift");
				final String titleDescription = rset.getString("title_description");
	...
	public void saveIngameShopItem(int objectId, int itemId, long itemCount, long itemPrice, byte category, byte subCategory, int list, int salesRanking, byte itemType, byte gift, String titleDescription, String description)
	{
		...
		{
			con = DatabaseFactory.getConnection();
			final PreparedStatement stmt = con.prepareStatement("INSERT INTO ingameshop(object_id, item_id, item_count, item_price, category, sub_category, list, sales_ranking, item_type, gift, title_description, description)" + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public boolean increaseSales(int object, int current)
	{
		...
		{
			con = DatabaseFactory.getConnection();
			final PreparedStatement stmt = con.prepareStatement(UPDATE_SALES_QUERY);
