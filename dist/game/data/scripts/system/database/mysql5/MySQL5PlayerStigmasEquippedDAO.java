...
public class MySQL5PlayerStigmasEquippedDAO extends PlayerStigmasEquippedDAO
{
	...
	{
		final List<EquippedStigmasEntry> items = new ArrayList<>();
		Connection con = null;
		try
		{
			...
			while (rset.next())
			{
				final int id = rset.getInt("item_id");
				final String name = rset.getString("item_name");
	...
	{
		final List<EquippedStigmasEntry> skillsActive = Lists.newArrayList(player.getEquipedStigmaList().getAllItems());
		final List<EquippedStigmasEntry> skillsDeleted = Lists.newArrayList(player.getEquipedStigmaList().getDeletedItems());
	...
	{
		final Collection<EquippedStigmasEntry> skillsToInsert = Collections2.filter(items, itemsToInsertPredicate);
	...
	{
		final Collection<EquippedStigmasEntry> skillsToUpdate = Collections2.filter(skills, itemsToUpdatePredicate);
	...
	{
		final Collection<EquippedStigmasEntry> skillsToDelete = Collections2.filter(skills, itemsToDeletePredicate);
