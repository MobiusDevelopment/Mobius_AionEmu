...
public class MySQL5PlayerCPDAO extends PlayerCPDAO
{
	private static final Logger log = LoggerFactory.getLogger(MySQL5PlayerCPDAO.class);
	...
	{
		final List<PlayerCPEntry> cp = new ArrayList<>();
		Connection con = null;
		try
		{
			...
			while (rset.next())
			{
				...
				final int point = rset.getInt("point");
	...
	public int getSlotSize(int playerObjId)
	{
		...
		{
			con = DatabaseFactory.getConnection();
			final PreparedStatement stmt = con.prepareStatement("SELECT COUNT(*) AS `size` FROM `player_cp` WHERE `player_id`=?");
	...
	public int getCPSlotObjId(final int obj)
	{
		...
		{
			con = DatabaseFactory.getConnection();
			final PreparedStatement s = con.prepareStatement("SELECT `slot` FROM `player_cp` WHERE `player_id`=?");
			s.setInt(1, obj);
