...
public class MySQL5ChallengeTasksDAO extends ChallengeTasksDAO
{
	...
	public Map<Integer, ChallengeTask> load(int ownerId, ChallengeType type)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				final int taskId = rset.getInt("task_id");
				final int questId = rset.getInt("quest_id");
				final int completeCount = rset.getInt("complete_count");
				final Timestamp date = rset.getTimestamp("complete_time");
				final ChallengeQuestTemplate template = DataManager.CHALLENGE_DATA.getQuestByQuestId(questId);
				final ChallengeQuest quest = new ChallengeQuest(template, completeCount);
				quest.setPersistentState(PersistentState.UPDATED);
				if (!tasks.containsKey(taskId))
				{
					final Map<Integer, ChallengeQuest> quests = new HashMap<>(2);
					quests.put(quest.getQuestId(), quest);
					final ChallengeTask task = new ChallengeTask(taskId, ownerId, quests, date);
