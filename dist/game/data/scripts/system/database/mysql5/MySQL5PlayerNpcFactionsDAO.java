...
public class MySQL5PlayerNpcFactionsDAO extends PlayerNpcFactionsDAO
{
	...
	public void loadNpcFactions(Player player)
	{
		...
		try
		{
			...
			final NpcFactions factions = new NpcFactions(player);
			player.setNpcFactions(factions);
			while (rset.next())
			{
				final int faction_id = rset.getInt("faction_id");
				final boolean active = rset.getBoolean("active");
				final int time = rset.getInt("time");
				final int questId = rset.getInt("quest_id");
				final ENpcFactionQuestState state = ENpcFactionQuestState.valueOf(rset.getString("state"));
				final NpcFaction faction = new NpcFaction(faction_id, time, active, state, questId);
