...
public class MySQL5ItemStoneListDAO extends ItemStoneListDAO
{
	...
	public void save(List<Item> items)
	{
		...
		final Set<ManaStone> manaStones = Sets.newHashSet();
	...
	private void store(Set<? extends ItemStone> stones, ItemStoneType ist)
	{
		...
		final Set<? extends ItemStone> stonesToAdd = Sets.filter(stones, itemStoneAddPredicate);
		final Set<? extends ItemStone> stonesToDelete = Sets.filter(stones, itemStoneDeletedPredicate);
		final Set<? extends ItemStone> stonesToUpdate = Sets.filter(stones, itemStoneUpdatePredicate);
		Connection con = null;
		try
		{
			con = DatabaseFactory.getConnection();
