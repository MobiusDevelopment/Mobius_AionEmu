...
public class LegionCommand extends AdminCommand
{
	...
	public void execute(Player player, String... params)
	{
		...
		if (params[0].equalsIgnoreCase("disband"))
		{
			...
			final Legion legion = verifyLegionExists(player, params[1]);
			if (legion == null)
			{
				return;
			}
		...
		else if (params[0].equalsIgnoreCase("setlevel"))
		{
			...
			final int old = legion.getLegionLevel();
		...
		else if (params[0].equalsIgnoreCase("setpoints"))
		{
			...
			final long old = legion.getContributionPoints();
		...
		else if (params[0].equalsIgnoreCase("info"))
		{
			...
			for (int memberId : members)
			{
				...
				else
				{
					...
					final PlayerCommonData pcd = dao.loadPlayerCommonData(memberId);
		...
		else if (params[0].equalsIgnoreCase("kick"))
		{
			...
			final Player target = World.getInstance().findPlayer(Util.convertName(params[1]));
			if (target == null)
			{
		...
		else if (params[0].equalsIgnoreCase("invite"))
		{
			...
			final Player target = World.getInstance().findPlayer(Util.convertName(params[2]));
			if (target == null)
			{
	...
	private boolean verifyLenght(Player player, int size, String... cmd)
	{
		...
		if (!ok)
		{
			onFail(player, size + " parameters required for element //legion " + cmd[0] + ".");
		}
