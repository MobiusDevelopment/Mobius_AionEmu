...
public class EventAi extends AdminCommand
{
	...
	public void execute(final Player admin, String... params)
	{
		...
		{
			final int rewardAmount = Integer.parseInt(params[2]);
			...
			if (params[1].equals("gp"))
			{
				...
				while (ita.hasNext())
				{
					...
					{
						// AbyssPointsService.addGp(player, 0, rewardAmount);
		...
		else if (params[0].equalsIgnoreCase("reward_range"))
		{
			...
			final RequestResponseHandler requestHim = new RequestResponseHandler(admin)
			{
				@Override
			...
			final boolean areyousure = admin.getResponseRequester().putRequest(1300564, requestHim);
			if (areyousure)
			{
		...
		else if (params[0].equalsIgnoreCase("stop"))
		{
			if (params[1].equalsIgnoreCase("all"))
			{
				...
				/*
				 * }else if(params[1].equalsIgnoreCase("near")){ Iterator<Player> ita = World.getInstance().getPlayersIterator(); while(ita.hasNext()){ Player player = ita.next(); if(player.getWorldId() == admin.getWorldId()){ } }
				 */
			}
			else
			{
		...
		{
			final Player player = World.getInstance().findPlayer(Util.convertName(params[1]));
