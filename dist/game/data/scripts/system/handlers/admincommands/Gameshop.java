...
public class Gameshop extends AdminCommand
{
	...
	public void execute(Player admin, String... params)
	{
		...
		else if ("add".startsWith(params[0]))
		{
			...
			DAOManager.getDAO(InGameShopDAO.class).saveIngameShopItem(IDFactory.getInstance().nextId(), itemId, count, price, category, subCategory, list - 1, 1, itemType, gift, titleDescription, description);
		...
		else if ("settoll".startsWith(params[0]))
		{
			if (params.length == 3)
			{
				...
				final String name = Util.convertName(params[1]);
