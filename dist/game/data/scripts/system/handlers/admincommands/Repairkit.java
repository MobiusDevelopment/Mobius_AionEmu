...
public class Repairkit extends AdminCommand
{
	...
	private final Logger log = LoggerFactory.getLogger("GM_MONITOR_LOG");
	...
	public void execute(Player admin, String... params)
	{
		...
		else if (params[0].equalsIgnoreCase("wipe"))
		{
			...
			final String playerToWipe = params[1];
	...
	private void removeItemFromDatabase(Player admin, int itemId)
	{
		...
		try
		{
			...
			final PreparedStatement stmt = con.prepareStatement("DELETE FROM inventory WHERE item_id=?");
	...
	private void wipeInventoryExceptEquiped(Player admin, String playerToWipe)
	{
		...
		{
			con = DatabaseFactory.getConnection();
			final PreparedStatement stmt = con.prepareStatement("DELETE FROM inventory WHERE item_owner=? AND is_equiped = 0");
	...
	private void removeItemByID(Player admin, String playerToRemoveFrom, int itemID)
	{
		...
		{
			con = DatabaseFactory.getConnection();
			final PreparedStatement stmt = con.prepareStatement("DELETE FROM inventory WHERE item_id=? AND item_owner=?");
			stmt.setInt(1, itemID);
	...
	private void returnPlayer(Player admin, String playerToReturn, String race)
	{
		...
		{
			con = DatabaseFactory.getConnection();
			final PreparedStatement stmt = con.prepareStatement("UPDATE players SET x=?, y=?, z=?, heading=?, world_id=? WHERE id=?");
