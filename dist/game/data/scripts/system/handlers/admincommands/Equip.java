...
public class Equip extends AdminCommand
{
		...
		for (Item targetItem : player.getEquipment().getEquippedItemsWithoutStigma())
		{
			if (isUpgradable(targetItem))
			{
				...
				else
				{
					...
					{
						final ManaStone manaStone = ItemSocketService.addManaStone(targetItem, manastone);
	...
	{
		final Item targetItem = player.getEquipment().getMainHandWeapon();
		if (godstone > 168000028)
		{
			final ItemTemplate itemTemplate = DataManager.ITEM_DATA.getItemTemplate(godstone);
		...
		else if (targetItem.getGodStone() != null)
		{
			...
			try
			{
				...
				final Class<?> gs = targetItem.getGodStone().getClass();
				final Field probability = gs.getDeclaredField("probability");
				final Field probabilityLeft = gs.getDeclaredField("probability");
	...
	private void enchant(Player admin, Player player, int enchant)
	{
		for (Item targetItem : player.getEquipment().getEquippedItemsWithoutStigma())
		{
			if (isUpgradable(targetItem))
			{
				...
				if (enchant > 20)
				{
					final int skillId = getRndSkills(targetItem);
	...
	public static boolean isTempering(Item item)
	{
		...
		{
			final int pt = item.getItemTemplate().getItemSlot();
			if ((pt == 1) || /* Main Hand */
				(pt == 2) || /* Sub Hand */
				(pt == 4) || /* Helmet */
				(pt == 8) || /* Jacket */
