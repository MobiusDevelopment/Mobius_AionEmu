...
public class FixPath extends AdminCommand
{
	...
	public void execute(final Player admin, String... params)
	{
		...
		ThreadPoolManager.getInstance().schedule(new Runnable()
		{
			...
			public void run()
			{
				final boolean wasInvul = admin.isInvul();
				admin.setInvul(true);
				
				float zDelta = 0;
				final HashMap<Integer, Float> corrections = new HashMap<>();
				...
				try
				{
					...
					final WalkerData data = new WalkerData();
					final WalkerTemplate newTemplate = new WalkerTemplate(template.getRouteId());
					
					i = 1;
					final ArrayList<RouteStep> newSteps = new ArrayList<>();
					
					final int lastStep = template.isReversed() ? (template.getRouteSteps().size() + 2) / 2 : template.getRouteSteps().size();
					for (int s = 0; s < lastStep; s++)
					{
						final RouteStep step = template.getRouteSteps().get(s);
						final RouteStep fixedStep = new RouteStep(step.getX(), step.getY(), corrections.get(i), 0);
