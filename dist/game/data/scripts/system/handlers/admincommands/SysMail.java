...
public class SysMail extends AdminCommand
{
	...
	public void execute(Player admin, String... params)
	{
		...
		final Boolean express = checkExpress(admin, item, count, kinah, recipient, recipientType, letterType);
		if (express == null)
		{
			return;
		}
		...
		{
			final String[] words = new String[paramValues.length - 5];
			System.arraycopy(paramValues, 5, words, 0, words.length);
			final String[] outText = new String[1];
			int wordCount = extractText(words, outText);
			if (wordCount > 0)
			{
				title = outText[0];
				final String[] msgWords = new String[words.length - wordCount];
	...
	private int extractText(String[] words, String[] outText)
	{
		...
		final String enclosedText = words[0].substring(1);
		...
		{
			final List<String> titleWords = new ArrayList<>();
	...
	private static Boolean checkExpress(Player admin, int item, int count, int kinah, String recipient, RecipientType recipientType, LetterType letterType)
	{
		...
		final ItemTemplate itemTemplate = DataManager.ITEM_DATA.getItemTemplate(item);
		if (item != 0)
		{
