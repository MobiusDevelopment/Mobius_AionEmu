...
public class Info extends AdminCommand
{
	...
	public void execute(Player admin, String... params)
	{
		...
		if (target instanceof Player)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				final NpcFaction faction = player.getNpcFactions().getActiveNpcFaction(i == 0);
				if (faction != null)
				{
		...
		else if (target instanceof Npc)
		{
			...
			{
				final SiegeNpc siegeNpc = (SiegeNpc) npc;
			...
			{
				final WalkerTemplate template = DataManager.WALKER_DATA.getWalkerTemplate(npc.getSpawn().getWalkerId());
				if (template != null)
				{
					...
					{
						final ClusteredNpc snpc = npc.getWalkerGroup().getClusterData(npc);
