...
public class PlayerInfo extends AdminCommand
{
	...
	public void execute(Player admin, String... params)
	{
		...
		{
			final StringBuilder strbld = new StringBuilder("-items in inventory:\n");
		...
		{
			final StringBuilder strbld = new StringBuilder("-list of skills:\n");
		...
		{
			final String chatLink = ChatUtil.position(target.getName(), target.getPosition());
		...
		else if (params[1].equals("legion"))
		{
			...
			final Legion legion = target.getLegion();
			if (legion == null)
			{
			...
			{
				final ArrayList<LegionMemberEx> legionmemblist = LegionService.getInstance().loadLegionMemberExList(legion, null);
				final Iterator<LegionMemberEx> it = legionmemblist.iterator();
		...
		else if (params[1].equals("chars"))
		{
			...
			final Iterator<PlayerAccountData> data = target.getClientConnection().getAccount().iterator();
			while (data.hasNext())
			{
				final PlayerAccountData d = data.next();
		...
		else if (params[1].equals("knownlist"))
		{
			...
			{
				final int objId = Integer.parseInt(params[3]);
				final VisibleObject obj = World.getInstance().findVisibleObject(objId);
