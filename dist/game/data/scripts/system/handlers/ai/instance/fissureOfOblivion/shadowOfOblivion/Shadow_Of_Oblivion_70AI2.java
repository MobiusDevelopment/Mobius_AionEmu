...
public class Shadow_Of_Oblivion_70AI2 extends AggressiveNpcAI2
{
	...
	private void ShadowOfOblivionType()
	{
		...
		{
			case 1:
				spawn(244655, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			case 2:
				spawn(244656, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			case 3:
				spawn(244657, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			case 4:
				spawn(244658, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
		}
	}
}