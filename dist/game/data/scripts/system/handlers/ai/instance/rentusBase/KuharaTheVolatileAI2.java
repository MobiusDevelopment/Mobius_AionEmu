...
public class KuharaTheVolatileAI2 extends AggressiveNpcAI2
{
	...
	{
		barrelEventTask = ThreadPoolManager.getInstance().scheduleAtFixedRate((Runnable) () ->
		{
			if (isAlreadyDead())
			{
	...
	{
		bombEventTask = ThreadPoolManager.getInstance().schedule((Runnable) () ->
		{
	...
	{
		activeEventTask = ThreadPoolManager.getInstance().scheduleAtFixedRate((Runnable) () ->
		{
			if (isAlreadyDead())
			{
				cancelActiveEventTask();
