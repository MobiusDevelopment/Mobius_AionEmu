...
public class Shadow_Of_Oblivion_75AI2 extends AggressiveNpcAI2
{
	...
	private void ShadowOfOblivionType()
	{
		...
		switch (Rnd.get(1, 4))
		{
			case 1:
			{
				spawn(244860, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			}
			case 2:
			{
				spawn(244861, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			}
			case 3:
			{
				spawn(244862, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			}
			case 4:
			{
				spawn(244863, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			}
		}
	}
