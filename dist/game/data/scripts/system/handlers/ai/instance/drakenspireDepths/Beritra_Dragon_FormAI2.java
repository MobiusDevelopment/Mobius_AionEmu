...
@AIName("beritra_dragon_form")
public class Beritra_Dragon_FormAI2 extends AggressiveNpcAI2
{
	private Future<?> dragonFormTask;
		...
		if (isAggred.compareAndSet(false, true))
		{
			switch (getNpcId())
			{
				case 236247:
					...
					dragonFormTask = ThreadPoolManager.getInstance().schedule(new Runnable()
					{
						@Override
						public void run()
						{
							AI2Actions.deleteOwner(Beritra_Dragon_FormAI2.this);
						}
					}, 420000);
					break;
			}
	...
	private void cancelDragonFormTask()
	{
		if ((dragonFormTask != null) && !dragonFormTask.isDone())
		{
			dragonFormTask.cancel(true);
		}
	}
