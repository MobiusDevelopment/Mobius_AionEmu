...
public class Shadow_Of_Oblivion_73AI2 extends AggressiveNpcAI2
{
	...
	private void ShadowOfOblivionType()
	{
		...
		switch (Rnd.get(1, 4))
		{
			case 1:
			{
				spawn(244778, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			}
			case 2:
			{
				spawn(244779, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			}
			case 3:
			{
				spawn(244780, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			}
			case 4:
			{
				spawn(244781, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
				break;
			}
		}
	}
