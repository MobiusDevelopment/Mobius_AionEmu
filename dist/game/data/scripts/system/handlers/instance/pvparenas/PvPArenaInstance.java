...
public class PvPArenaInstance extends GeneralInstanceHandler
{
	...
	{
		final PvPArenaPlayerReward ownerReward = getPlayerReward(player.getObjectId());
		...
		if ((lastAttacker != null) && (lastAttacker != player))
		{
			if (lastAttacker instanceof Player)
			{
				...
				final PvPArenaPlayerReward reward = getPlayerReward(winner.getObjectId());
	...
	private void updatePoints(Creature victim)
	{
		...
		{
			final PvPArenaPlayerReward victimFine = getPlayerReward(victim.getObjectId());
	...
	protected void reward()
	{
		for (Player player : instance.getPlayersInside())
		{
			...
			final PvPArenaPlayerReward reward = getPlayerReward(player.getObjectId());
			if (!reward.isRewarded())
			{
				...
				final int courage = reward.getBasicCourage() + reward.getRankingCourage() + reward.getScoreCourage();
				if (courage != 0)
				{
					ItemService.addItem(player, 186000137, courage);
				}
				final int crucible = reward.getBasicCrucible() + reward.getRankingCrucible() + reward.getScoreCrucible();
				if (crucible != 0)
				{
					ItemService.addItem(player, 186000130, crucible);
				}
				final int opportunity = reward.getOpportunity();
				if (opportunity != 0)
				{
					ItemService.addItem(player, 186000165, opportunity);
				}
				final int gloryTicket = reward.getGloryTicket();
				if (gloryTicket != 0)
				{
					ItemService.addItem(player, 186000185, gloryTicket);
				}
				final int mithrilMedal = reward.getMithrilMedal();
				if (mithrilMedal != 0)
				{
					ItemService.addItem(player, 186000147, mithrilMedal);
				}
				final int platinumMedal = reward.getPlatinumMedal();
				if (platinumMedal != 0)
				{
					ItemService.addItem(player, 186000096, platinumMedal);
				}
				final int gloriousInsignia = reward.getGloriousInsignia();
				if (gloriousInsignia != 0)
				{
					ItemService.addItem(player, 182213259, gloriousInsignia);
				}
				final int lifeSerum = reward.getLifeSerum();
				if (lifeSerum != 0)
				{
					ItemService.addItem(player, 162000077, lifeSerum);
				}
				final int infinity = reward.getBasicInfinity() + reward.getRankingInfinity() + reward.getScoreInfinity();
				if (infinity != 0)
				{
