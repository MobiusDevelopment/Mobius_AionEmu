...
/**
 * Author Rinzler (Encom) Walkthrough: After entering the instance, there is a blue Aether barrier. Once the Aetheric barrier is broken, there are 15 minutes until the treasure chests vanish. All mobs are elite (except the Drakies) in this instance, and give AP. There are many small chests that may
 * contain AP relics, medals, kinah or quest items. Also there is a Golden chest that spawns at one of the four possible locations and may contain level 30 AP items, relics or kinah. Pesky Drakies will prevent you from opening chests, so either kill them or use Anti-Shock scrolls and damage
 * absorbing skills to ignore the drakies. Pull mobs very carefully and watch out for patrols. If there is a level 44+ player in your group, who isn't mentoring, chests won't drop anything except medals and quest items. Note!!! that relics and abyss items aren't tradeable, not even temporary.
 **/
...
public class RightWingChamberInstance extends GeneralInstanceHandler
{
	...
	private void startRightWingChamberTimer()
	{
		instance.doOnAllPlayers(player ->
		{
			if (player.isOnline())
			{
				rightWingTask = ThreadPoolManager.getInstance().schedule(() ->
				{
	...
	public void onEnterInstance(Player player)
	{
		...
		if (!isStartTimer)
		{
			...
			chestRWCTask = ThreadPoolManager.getInstance().schedule(() ->
			{
	...
	public boolean onReviveEvent(Player player)
	{
		...
		return TeleportService2.teleportTo(player, mapId, instanceId, 265.984f, 387.77493f, 103.24291f, (byte) 0);
	}
