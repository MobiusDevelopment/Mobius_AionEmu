...
public class DarkPoetaInstance extends GeneralInstanceHandler
{
		...
		switch (npcId)
		{
			...
			case 215280: // Tahabata Pyrelord.
			{
				for (Player player : instance.getPlayersInside())
				{
					if (player.isOnline())
					{
						dropItems.add(DropRegistrationService.getInstance().regDropItem(1, 0, npcId, 188052951, 1)); // [Event] Prestige Supplies.
	...
	public void onGather(Player player, Gatherable gatherable)
	{
		...
		PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1400237, name, 157));
	}
	...
	public boolean onReviveEvent(Player player)
	{
		...
		PacketSendUtility.sendPacket(player, new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_INSTANT_DUNGEON_IDLF1_RESURRECT, 0, 0));
		PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_REBIRTH_MASSAGE_ME);
		return TeleportService2.teleportTo(player, mapId, instanceId, 1225.5182f, 419.65796f, 140.18988f, (byte) 71);
	}
