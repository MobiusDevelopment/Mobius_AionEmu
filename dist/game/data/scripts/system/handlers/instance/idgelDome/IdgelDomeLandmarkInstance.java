 ...
 */
package system.handlers.instance.idgelDome;
...
public class IdgelDomeLandmarkInstance extends GeneralInstanceHandler
{
	...
	protected void reward()
	{
		...
		for (Player player : instance.getPlayersInside())
		{
			...
			final LandMarkPlayerReward playerReward = landMarkReward.getPlayerReward(player.getObjectId());
			int abyssPoint = 3163;
			int gloryPoint = 150;
			int expPoint = 10000;
			...
			if (player.getRace().equals(landMarkReward.getWinnerRace()))
			{
				...
				playerReward.setAdditionalReward(188055396); // 유적지 최상급 보상 상자.
			}
			else
			{
			...
			ItemService.addItem(player, 188055396, 1); // 유적지 최상급 보상 상자.
	...
	{
		final LandMarkPlayerReward ownerReward = landMarkReward.getPlayerReward(player.getObjectId());
		...
		if (lastAttacker instanceof Player)
		{
			...
			{
				final InstancePlayerReward playerReward = landMarkReward.getPlayerReward(player.getObjectId());
	...
	private void clearLandMarkDebuffs(Player player)
	{
