...
public class cmd_reskin2 extends PlayerCommand
{
	...
	public void execute(Player admin, String... params)
	{
		...
		try
		{
			...
			try
			{
				...
				if (items.equals("[item:"))
				{
					items = params[2];
					final Pattern id = Pattern.compile("(\\d{9})");
					final Matcher result = id.matcher(items);
					if (result.find())
					{
		...
		final int tollPrice = 50;
		final List<Item> items = target.getInventory().getItemsByItemId(oldItemId);
		final List<Item> itemnew = target.getInventory().getItemsByItemId(newItemId);
	...
	public void reskin(Player admin, int toll, int itemId, List<Item> items)
	{
		...
		final boolean requested = admin.getResponseRequester().putRequest(902247, responseHandler);
		if (requested)
		{
