 ...
 * operations are thread-safe, retrieval operations do <em>not</em> entail locking, and there is <em>not</em> any support for locking the entire table in a way that prevents all access. This class is fully inter-operable with {@code Hashtable} in programs that rely on its thread safety but not on
 * its synchronization details.
 * <p/>
 * <p/>
 * Retrieval operations (including {@code get}) generally do not block, so may overlap with update operations (including {@code put} and {@code remove}). Retrievals reflect the results of the most recently <em>completed</em> update operations holding upon their onset. (More formally, an update
 * operation for a given key bears a <em>happens-before</em> relation with any (non-null) retrieval for that key reporting the updated value.) For aggregate operations such as {@code putAll} and {@code clear}, concurrent retrievals may reflect insertion or removal of only some entries. Similarly,
 * Iterators and Enumerations return elements reflecting the state of the hash table at some point at or since the creation of the iterator/enumeration. They do <em>not</em> throw {@link ConcurrentModificationException}. However, iterators are designed to be used by only one thread at a time. Bear
 * in mind that the results of aggregate status methods including {@code size}, {@code
 * isEmpty}, and {@code containsValue} are typically useful only when a map is not undergoing concurrent updates in other threads. Otherwise the results of these methods reflect transient states that may be adequate for monitoring or estimation purposes, but not for program control.
 * <p/>
 * <p/>
 * The table is dynamically expanded when there are too many collisions (i.e., keys that have distinct hash codes but fall into the same slot modulo the table size), with the expected average effect of maintaining roughly two bins per mapping (corresponding to a 0.75 load factor threshold for
 * resizing). There may be much variance around this average as mappings are added and removed, but overall, this maintains a commonly accepted time/space tradeoff for hash tables. However, resizing this or any other kind of hash table may be a relatively slow operation. When possible, it is a good
 * idea to provide a size estimate as an optional {@code
 * initialCapacity} constructor argument. An additional optional {@code loadFactor} constructor argument provides a further means of customizing initial table capacity by specifying the table density to be used in calculating the amount of space to allocate for the given number of elements. Also,
 * for compatibility with previous versions of this class, constructors may optionally specify an expected {@code concurrencyLevel} as an additional hint for internal sizing. Note that using many keys with exactly the same {@code hashCode()} is a sure way to slow down performance of any hash table.
 * <p/>
 * <p/>
 ...
 * <p/>
 * <p/>
 * A ConcurrentHashMapV8 can be used as scalable frequency map (a form of histogram or multiset) by using {@link LongAdder} values and initializing via {@link #computeIfAbsent}. For example, to add a count to a {@code ConcurrentHashMapV8<String,LongAdder> freqs}, you can use
 ...
 * <p/>
 * <p/>
 * ConcurrentHashMapV8s support sequential and parallel operations bulk operations. (Parallel forms use the {@link ForkJoinPool#commonPool()}). Tasks that may be used in other contexts are available in class {@link ForkJoinTasks}. These operations are designed to be safely, and often sensibly,
 * applied even with maps that are being concurrently updated by other threads; for example, when computing a snapshot summary of the values in a shared registry. There are three kinds of operation, each with four forms, accepting functions with Keys, Values, Entries, and (Key, Value) arguments
 * and/or return values. Because the elements of a ConcurrentHashMapV8 are not ordered in any particular way, and may be processed in different orders in different parallel executions, the correctness of supplied functions should not depend on any ordering, or on any other objects or values that may
 * transiently change while computation is in progress; and except for forEach actions, should ideally be side-effect-free.
 * <p/>
 * <ul>
 ...
 * <p/>
 * <p/>
 * The concurrency properties of bulk operations follow from those of ConcurrentHashMapV8: Any non-null result returned from {@code get(key)} and related access methods bears a happens-before relation with the associated insertion or update. The result of any bulk operation reflects the composition
 * of these per-element relations (but is not necessarily atomic with respect to the map as a whole unless it is somehow known to be quiescent). Conversely, because keys and values in the map are never null, null serves as a reliable atomic indicator of the current lack of any result. To maintain
 * this property, null serves as an implicit basis for all non-scalar reduction operations. For the double, long, and int versions, the basis should be one that, when combined with any other value, returns that other value (more formally, it should be the identity element for the reduction). Most
 * common reductions have these properties; for example, computing a sum with basis 0 or a minimum with basis MAX_VALUE.
 * <p/>
 * <p/>
 * Search and transformation functions provided as arguments should similarly return null to indicate the lack of any result (in which case it is not used). In the case of mapped reductions, this also enables transformations to serve as filters, returning null (or, in the case of primitive
 * specializations, the identity basis) if the element should not be combined. You can create compound transformations and filterings by composing them yourself under this "null means there is nothing there now" rule before using them in search or reduce operations.
 * <p/>
 * <p/>
 ...
 * <p/>
 * <p/>
 * Bulk operations may complete abruptly, throwing an exception encountered in the application of a supplied function. Bear in mind when handling such exceptions that other concurrently executing functions could also have thrown exceptions, or would have done so if the first exception had not
 * occurred.
 * <p/>
 * <p/>
 * Speedups for parallel compared to sequential forms are common but not guaranteed. Parallel operations involving brief functions on small maps may execute more slowly than sequential forms if the underlying work to parallelize the computation is more expensive than the computation itself.
 ...
 * <p/>
 * <p/>
 * <em>jsr166e note: During transition, this class uses nested functional interfaces with different names but the same forms as those expected for JDK8.</em>
 * <p/>
 * <p/>
...
public class ConcurrentHashMapV8<K, V> implements ConcurrentMap<K, V>, Serializable
{
	...
	public static interface Spliterator<T>extends Iterator<T>
	{
		...
		/**
		 * Returns a Spliterator covering approximately half of the elements, guaranteed not to overlap with those subsequently returned by this Spliterator. After invoking this method, the current Spliterator will <em>not</em> produce any of the elements of the returned Spliterator, but the two
		 * Spliterators together will produce all of the elements that would have been produced by this Spliterator had this method not been called. The exact number of elements produced by the returned Spliterator is not guaranteed, and may be zero (i.e., with {@code hasNext()} reporting {@code
		 * false}) if this Spliterator cannot be further split.
		 * @return a Spliterator covering approximately half of the elements
		 * @throws IllegalStateException if this Spliterator has already commenced traversing elements
		 */
		Spliterator<T> split();
	}
	
	/*
	 * Overview: The primary design goal of this hash table is to maintain concurrent readability (typically method get(), but also iterators and related methods) while minimizing update contention. Secondary goals are to keep space consumption about the same or better than java.util.HashMap, and to
	 * support high initial insertion rates on an empty table by many threads. Each key-value mapping is held in a Node. Because Node key fields can contain special values, they are defined using plain Object types (not type "K"). This leads to a lot of explicit casting (and many explicit warning
	 * suppressions to tell compilers not to complain about it). It also allows some of the public methods to be factored into a smaller number of internal methods (although sadly not so for the five variants of put-related operations). The validation-based approach explained below leads to a lot of
	 * code sprawl because retry-control precludes factoring into smaller methods. The table is lazily initialized to a power-of-two size upon the first insertion. Each bin in the table normally contains a list of Nodes (most often, the list has only zero or one Node). Table accesses require
	 * volatile/atomic reads, writes, and CASes. Because there is no other way to arrange this without adding further indirections, we use intrinsics (sun.misc.Unsafe) operations. The lists of nodes within bins are always accurately traversable under volatile reads, so long as lookups check hash
	 * code and non-nullness of value before checking key equality. We use the top (sign) bit of Node hash fields for control purposes -- it is available anyway because of addressing constraints. Nodes with negative hash fields are forwarding nodes to either TreeBins or resized tables. The lower 31
	 * bits of each normal Node's hash field contain a transformation of the key's hash code. Insertion (via put or its variants) of the first node in an empty bin is performed by just CASing it to the bin. This is by far the most common case for put operations under most key/hash distributions.
	 * Other update operations (insert, delete, and replace) require locks. We do not want to waste the space required to associate a distinct lock object with each bin, so instead use the first node of a bin list itself as a lock. Locking support for these locks relies on builtin "synchronized"
	 * monitors. Using the first node of a list as a lock does not by itself suffice though: When a node is locked, any update must first validate that it is still the first node after locking it, and retry if not. Because new nodes are always appended to lists, once a node is first in a bin, it
	 * remains first until deleted or the bin becomes invalidated (upon resizing). However, operations that only conditionally update may inspect nodes until the point of update. This is a converse of sorts to the lazy locking technique described by Herlihy & Shavit. The main disadvantage of per-bin
	 * locks is that other update operations on other nodes in a bin list protected by the same lock can stall, for example when user equals() or mapping functions take a long time. However, statistically, under random hash codes, this is not a common problem. Ideally, the frequency of nodes in bins
	 * follows a Poisson distribution (http://en.wikipedia.org/wiki/Poisson_distribution) with a parameter of about 0.5 on average, given the resizing threshold of 0.75, although with a large variance because of resizing granularity. Ignoring variance, the expected occurrences of list size k are
	 * (exp(-0.5) * pow(0.5, k) / factorial(k)). The first values are: 0: 0.60653066 1: 0.30326533 2: 0.07581633 3: 0.01263606 4: 0.00157952 5: 0.00015795 6: 0.00001316 7: 0.00000094 8: 0.00000006 more: less than 1 in ten million Lock contention probability for two threads accessing distinct
	 * elements is roughly 1 / (8 * #elements) under random hashes. Actual hash code distributions encountered in practice sometimes deviate significantly from uniform randomness. This includes the case when N > (1<<30), so some keys MUST collide. Similarly for dumb or hostile usages in which
	 * multiple keys are designed to have identical hash codes. Also, although we guard against the worst effects of this (see method spread), sets of hashes may differ only in bits that do not impact their bin index for a given power-of-two mask. So we use a secondary strategy that applies when the
	 * number of nodes in a bin exceeds a threshold, and at least one of the keys implements Comparable. These TreeBins use a balanced tree to hold nodes (a specialized form of red-black trees), bounding search time to O(log N). Each search step in a TreeBin is around twice as slow as in a regular
	 * list, but given that N cannot exceed (1<<64) (before running out of addresses) this bounds search steps, lock hold times, etc, to reasonable constants (roughly 100 nodes inspected per operation worst case) so long as keys are Comparable (which is very common -- String, Long, etc). TreeBin
	 * nodes (TreeNodes) also maintain the same "next" traversal pointers as regular nodes, so can be traversed in iterators in the same way. The table is resized when occupancy exceeds a percentage threshold (nominally, 0.75, but see below). Any thread noticing an overfull bin may assist in
	 * resizing after the initiating thread allocates and sets up the replacement array. However, rather than stalling, these other threads may proceed with insertions etc. The use of TreeBins shields us from the worst case effects of overfilling while resizes are in progress. Resizing proceeds by
	 * transferring bins, one by one, from the table to the next table. To enable concurrency, the next table must be (incrementally) prefilled with place-holders serving as reverse forwarders to the old table. Because we are using power-of-two expansion, the elements from each bin must either stay
	 * at same index, or move with a power of two offset. We eliminate unnecessary node creation by catching cases where old nodes can be reused because their next fields won't change. On average, only about one-sixth of them need cloning when a table doubles. The nodes they replace will be garbage
	 * collectable as soon as they are no longer referenced by any reader thread that may be in the midst of concurrently traversing table. Upon transfer, the old table bin contains only a special forwarding node (with hash field "MOVED") that contains the next table as its key. On encountering a
	 * forwarding node, access and update operations restart, using the new table. Each bin transfer requires its bin lock, which can stall waiting for locks while resizing. However, because other threads can join in and help resize rather than contend for locks, average aggregate waits become
	 * shorter as resizing progresses. The transfer operation must also ensure that all accessible bins in both the old and new table are usable by any traversal. This is arranged by proceeding from the last bin (table.length - 1) up towards the first. Upon seeing a forwarding node, traversals (see
	 * class Traverser) arrange to move to the new table without revisiting nodes. However, to ensure that no intervening nodes are skipped, bin splitting can only begin after the associated reverse-forwarders are in place. The traversal scheme also applies to partial traversals of ranges of bins
	 * (via an alternate Traverser constructor) to support partitioned aggregate operations. Also, read-only operations give up if ever forwarded to a null table, which provides support for shutdown-style clearing, which is also not currently implemented. Lazy table initialization minimizes
	 * footprint until first use, and also avoids resizings when the first operation is from a putAll, constructor with map argument, or deserialization. These cases attempt to override the initial capacity settings, but harmlessly fail to take effect in cases of races. The element count is
	 * maintained using a specialization of LongAdder. We need to incorporate a specialization rather than just use a LongAdder in order to access implicit contention-sensing that leads to creation of multiple CounterCells. The counter mechanics avoid contention on updates but can encounter cache
	 * thrashing if read too frequently during concurrent access. To avoid reading so often, resizing under contention is attempted only upon adding to a bin already holding two or more nodes. Under uniform hash distributions, the probability of this occurring at threshold is around 13%, meaning
	 * that only about 1 in 8 puts check threshold (and after resizing, many fewer do so). The bulk putAll operation further reduces contention by only committing count updates upon these size checks. Maintaining API and serialization compatibility with previous versions of this class introduces
	 * several oddities. Mainly: We leave untouched but unused constructor arguments refering to concurrencyLevel. We accept a loadFactor constructor argument, but apply it only to initial table capacity (which is the only time that we can guarantee to honor it.) We also declare an unused "Segment"
	...
	/**
	 * The bin count threshold for using a tree rather than list for a bin. The value reflects the approximate break-even point for using tree-based operations.
	 */
	...
	/**
	 * Table initialization and resizing control. When negative, the table is being initialized or resized: -1 for initialization, else -(1 + the number of active resizing threads). Otherwise, when table is null, holds the initial table size to use upon creation, or 0 for default. After
	...
	/*
	 * Volatile access methods are used for table elements as well as elements of in-progress next table while resizing. Uses are null checked by callers, and implicitly bounds-checked, relying on the invariants that tab arrays have non-zero size, and all indices are masked with (tab.length - 1)
	...
	/**
	 * A specialized form of red-black tree for use in bins whose size exceeds a threshold.
	 * <p/>
	 * TreeBins use a special form of comparison for search and related operations (which is the main reason we cannot use existing collections such as TreeMaps). TreeBins contain Comparable elements, but may contain others, as well as elements that are Comparable but not necessarily Comparable<T>
	 * for the same T, so we cannot invoke compareTo among them. To handle this, the tree is ordered primarily by hash value, then by getClass().getName() order, and then by Comparator order among elements of the same class. On lookup at a node, if elements are not comparable or compare as 0, both
	 * left and right children may need to be searched in the case of tied hash values. (This corresponds to the full list search that would be necessary if all elements were non-Comparable and had tied hashes.) The red-black balancing code is updated from pre-jdk-collections
	 ...
	 * <p/>
	 * TreeBins also maintain a separate locking discipline than regular bins. Because they are forwarded via special MOVED nodes at bin heads (which can never change once established), we cannot use those nodes as locks. Instead, TreeBin extends AbstractQueuedSynchronizer to support a simple form
	...
	static final class TreeBin<V>extends AbstractQueuedSynchronizer
	{
		...
		private void rotateLeft(TreeNode<V> p)
		{
			if (p != null)
			{
				...
				TreeNode<V> pp, rl = p.right = r.left;
				if (rl != null)
				{
					rl.parent = p;
				}
		...
		private void rotateRight(TreeNode<V> p)
		{
			if (p != null)
			{
				...
				TreeNode<V> pp, lr = p.left = l.right;
				if (lr != null)
				{
					lr.parent = p;
				}
			...
			while (p != null)
			{
				...
				if (ph == h)
				{
					...
					if ((c != pc) || !(k instanceof Comparable) || ((dir = ((Comparable<Object>) k).compareTo(pk)) == 0))
					{
						...
						if (dir == 0)
						{
							TreeNode<V> r = null, pl, pr = p.right; // check both sides
							if ((pr != null) && (h >= pr.hash) && ((r = getTreeNode(h, k, pr)) != null))
							{
								return r;
							}
		...
		final TreeNode<V> putTreeNode(int h, Object k, V v)
		{
			...
			final TreeNode<V> f = first;
			TreeNode<V> x = first = new TreeNode<>(h, k, v, f, p);
			if (p == null)
			{
				root = x;
			}
		...
		{
			final TreeNode<V> next = (TreeNode<V>) p.next; // unlink traversal pointers
			...
			{ // rebalance, from CLR
				...
				while (x != null)
				{
					...
					if (x == xpl)
					{
						...
						{
							final TreeNode<V> sl = sib.left;
							TreeNode<V> sr = sib.right;
					...
					{ // symmetric
						...
						{
							TreeNode<V> sl = sib.left;
							final TreeNode<V> sr = sib.right;
	...
	/*
	 * Internal versions of insertion methods All have the same basic structure as the first (internalPut): 1. If table uninitialized, create 2. If bin empty, try to CAS new node 3. If bin stale, use new table 4. if bin converted to TreeBin, validate and relay to TreeBin methods 5. Lock and
	 * validate; if valid, scan and add or update The putAll method differs mainly in attempting to pre-allocate enough table space, and also more lazily performs count updates and checks. Most of the function-accepting methods can't be factored nicely because they require different functional
	...
	final V internalPut(K k, V v, boolean onlyIfAbsent)
	{
		...
		for (Node<V>[] tab = table;;)
		{
			...
			else if (onlyIfAbsent && (fh == h) && ((fv = f.val) != null) && (((fk = f.key) == k) || k.equals(fk))) // peek while nearby
			{
				return fv;
			}
			else
	...
	private final V internalComputeIfAbsent(K k, Fun<? super K, ? extends V> mf)
	{
		...
		for (Node<V>[] tab = table;;)
		{
			...
			{
				final Node<V> node = new Node<>(h, k, null, null);
				synchronized (node)
				{
	...
	private final void transfer(Node<V>[] tab, Node<V>[] nextTab)
	{
		...
		final Node<V> fwd = new Node<>(MOVED, nextTab, null, null);
		...
		for (int i = 0, bound = 0;;)
		{
			...
			else if ((fk = f.key) instanceof TreeBin)
			{
				...
				try
				{
					if (tabAt(tab, i) == f)
					{
						final TreeBin<V> lt = new TreeBin<>();
						final TreeBin<V> ht = new TreeBin<>();
						int lc = 0, hc = 0;
	...
	/**
	 * Encapsulates traversal for methods such as containsValue; also serves as a base class for other iterators and bulk tasks.
	 * <p/>
	 ...
	 * of bookkeeping state that is difficult to optimize away amidst volatile accesses. Even so, traversal maintains reasonable throughput.
	 * <p/>
	 * Normally, iteration proceeds bin-by-bin traversing lists. However, if the table has been resized, then all future steps must traverse both the bin at the current index as well as at (index + baseSize); and so on for further resizings. To paranoically cope with potential sharing by users of
	 * iterators across threads, iteration terminates if a bounds checks fails for a table read.
	 * <p/>
	...
	static class Traverser<K, V, R>extends CountedCompleter<R>
	{
		...
		Traverser(ConcurrentHashMapV8<K, V> map, Traverser<K, V, ?> it, int batch)
		{
			...
			{ // split parent
				...
				final int hi = this.baseLimit = it.baseLimit;
		...
		final int preSplit()
		{
			...
			{ // force initialization
				...
				if (t != null)
				{
					...
					final int par = ((pool = getPool()) == null) ? ForkJoinPool.getCommonPoolParallelism() : pool.getParallelism();
					final int sp = par << 3; // slack of 8
	...
	/**
	 * Creates a new, empty map with an initial table size based on the given number of elements ({@code initialCapacity}) and initial table density ({@code loadFactor}).
	...
	/**
	 * Creates a new, empty map with an initial table size based on the given number of elements ({@code initialCapacity}), table density ({@code loadFactor}), and number of concurrently updating threads ({@code concurrencyLevel}).
	 * @param initialCapacity the initial capacity. The implementation performs internal sizing to accommodate this many elements, given the specified load factor.
	 * @param loadFactor the load factor (table density) for establishing the initial table size
	...
	{
		return new KeySetView<>(new ConcurrentHashMapV8<K, Boolean>(), Boolean.TRUE);
	}
	...
	{
		return new KeySetView<>(new ConcurrentHashMapV8<K, Boolean>(initialCapacity), Boolean.TRUE);
	}
	...
	/**
	 * Returns the number of mappings. This method should be used instead of {@link #size} because a ConcurrentHashMapV8 may contain more mappings than can be represented as an int. The value returned is an estimate; the actual count may differ if there are concurrent insertions or removals.
	...
	/**
	 * Returns {@code true} if this map maps one or more keys to the specified value. Note: This method may require a full traversal of the map, and is much slower than method {@code containsKey}.
	...
	public boolean containsValue(Object value)
	{
		...
		V v;
		final Traverser<K, V, Object> it = new Traverser<>(this);
	...
	/**
	 * Legacy method testing if some key maps into the specified value in this table. This method is identical in functionality to {@link #containsValue}, and exists solely to ensure full compatibility with class {@link java.util.Hashtable}, which supported this method prior to introduction of the
	 * Java Collections framework.
	 * @param value a value to search for
	 * @return {@code true} if and only if some key maps to the {@code value} argument in this table as determined by the {@code equals} method; {@code false} otherwise
	 * @throws NullPointerException if the specified value is null
	 */
	@Deprecated
	...
	/**
	 * If the specified key is not already associated with a value, computes its value using the given mappingFunction and enters it into the map unless null. This is equivalent to
	 * 
	 * <pre>
	 *  {@code
	...
	/**
	 * Computes a new mapping value given a key and its current mapped value (or {@code null} if there is no current mapping). This is equivalent to
	 * 
	 * <pre>
	 *  {@code
	 *   value = remappingFunction.apply(key, map.get(key));
	...
	{
		final KeySetView<K, V> ks = keySet;
		return (ks != null) ? ks : (keySet = new KeySetView<>(this, null));
	}
	
	/**
	 * Returns a {@link Set} view of the keys in this map, using the given common mapped value for any additions (i.e., {@link Collection#add} and {@link Collection#addAll}). This is of course only appropriate if it is acceptable to use the same value for all additions from this view.
	...
	/**
	 * Returns a {@link Collection} view of the values contained in this map. The collection is backed by the map, so changes to the map are reflected in the collection, and vice-versa.
	 */
	@Override
	public ValuesView<K, V> values()
	{
		final ValuesView<K, V> vs = values;
		return (vs != null) ? vs : (values = new ValuesView<>(this));
	}
	
	/**
	 * Returns a {@link Set} view of the mappings contained in this map. The set is backed by the map, so changes to the map are reflected in the set, and vice-versa. The set supports element removal, which removes the corresponding mapping from the map, via the {@code Iterator.remove},
	...
	{
		final EntrySetView<K, V> es = entrySet;
	...
	/**
	 * Returns the hash code value for this {@link Map}, i.e., the sum of, for each key-value pair in the map, {@code key.hashCode() ^ value.hashCode()}.
	...
	/**
	 * Compares the specified object with this map for equality. Returns {@code true} if the given object is a map with the same mappings as this map. This operation may return misleading results if either map is concurrently modified during execution of this method.
	...
	static final class MapEntry<K, V> implements Map.Entry<K, V>
	{
		...
		/**
		 * Sets our entry's value and writes through to the map. The value to return is somewhat arbitrary here. Since we do not necessarily track asynchronous changes, the most recent "previous" value could be different from what we return (or could even have been removed in which case the put will
				...
				try
				{
					if (table == null)
					{
						...
						while (p != null)
						{
							...
							final Node<V> q = p.next = tabAt(tab, j);
							setTabAt(tab, j, p);
	...
	public <U> void forEachSequentially(BiFun<? super K, ? super V, ? extends U> transformer, Action<U> action)
	{
		...
		while ((v = it.advance()) != null)
		{
			u = transformer.apply((K) it.nextKey, v);
			if (u != null)
			{
				action.apply(u);
			}
	...
	public <U> U searchSequentially(BiFun<? super K, ? super V, ? extends U> searchFunction)
	{
		...
		while ((v = it.advance()) != null)
		{
			u = searchFunction.apply((K) it.nextKey, v);
			if (u != null)
			{
				return u;
			}
	...
	public <U> void forEachKeySequentially(Fun<? super K, ? extends U> transformer, Action<U> action)
	{
		...
		while (it.advance() != null)
		{
			u = transformer.apply((K) it.nextKey);
			if (u != null)
			{
				action.apply(u);
			}
	...
	public <U> U searchKeysSequentially(Fun<? super K, ? extends U> searchFunction)
	{
		...
		while (it.advance() != null)
		{
			u = searchFunction.apply((K) it.nextKey);
			if (u != null)
			{
				return u;
			}
	...
	public <U> void forEachEntrySequentially(Fun<Map.Entry<K, V>, ? extends U> transformer, Action<U> action)
	{
		...
		while ((v = it.advance()) != null)
		{
			u = transformer.apply(entryFor((K) it.nextKey, v));
			if (u != null)
			{
				action.apply(u);
			}
	...
	public <U> U searchEntriesSequentially(Fun<Map.Entry<K, V>, ? extends U> searchFunction)
	{
		...
		while ((v = it.advance()) != null)
		{
			u = searchFunction.apply(entryFor((K) it.nextKey, v));
			if (u != null)
			{
				return u;
			}
	...
	public Map.Entry<K, V> reduceEntriesSequentially(BiFun<Map.Entry<K, V>, Map.Entry<K, V>, ? extends Map.Entry<K, V>> reducer)
	{
		...
		while ((v = it.advance()) != null)
		{
			final Map.Entry<K, V> u = entryFor((K) it.nextKey, v);
			r = (r == null) ? u : reducer.apply(r, u);
		}
		return r;
	}
	...
	public static class KeySetView<K, V>extends CHMView<K, V> implements Set<K>, java.io.Serializable
	{
		...
		/**
		 * Returns a "weakly consistent" iterator that will never throw {@link ConcurrentModificationException}, and guarantees to traverse elements as they existed upon construction of the iterator, and may (but is not guaranteed to) reflect any modifications subsequent to construction.
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Collection} of values, in which additions are disabled. This class cannot be directly instantiated. See {@link #values()}.
	 * <p/>
	 * <p/>
	 * The view's {@code iterator} is a "weakly consistent" iterator that will never throw {@link ConcurrentModificationException}, and guarantees to traverse elements as they existed upon construction of the iterator, and may (but is not guaranteed to) reflect any modifications subsequent to
	 * construction.
	...
	public static final class ValuesView<K, V>extends CHMView<K, V> implements Collection<V>
	{
		...
		public final boolean remove(Object o)
		{
			if (o != null)
			{
				final Iterator<V> it = new ValueIterator<>(map);
				while (it.hasNext())
				{
					if (o.equals(it.next()))
					{
	...
	/**
	 * Predefined tasks for performing bulk parallel operations on ConcurrentHashMapV8s. These tasks follow the forms and rules used for bulk operations. Each method has the same name, but returns a task rather than invoking it. These methods may be useful in custom applications such as submitting a
	 * task without waiting for completion, using a custom pool, or combining with other tasks.
	 */
	public static class ForkJoinTasks
	{
		...
		public static <K, V, U> ForkJoinTask<Void> forEach(ConcurrentHashMapV8<K, V> map, BiFun<? super K, ? super V, ? extends U> transformer, Action<U> action)
		{
			...
			return new ForEachTransformedMappingTask<>(map, null, -1, transformer, action);
		}
		...
		public static <K, V, U> ForkJoinTask<U> search(ConcurrentHashMapV8<K, V> map, BiFun<? super K, ? super V, ? extends U> searchFunction)
		{
			...
			return new SearchMappingsTask<>(map, null, -1, searchFunction, new AtomicReference<U>());
		}
		...
		public static <K, V, U> ForkJoinTask<U> reduce(ConcurrentHashMapV8<K, V> map, BiFun<? super K, ? super V, ? extends U> transformer, BiFun<? super U, ? super U, ? extends U> reducer)
		{
			...
			return new MapReduceMappingsTask<>(map, null, -1, null, transformer, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Double> reduceToDouble(ConcurrentHashMapV8<K, V> map, ObjectByObjectToDouble<? super K, ? super V> transformer, double basis, DoubleByDoubleToDouble reducer)
		{
			...
			return new MapReduceMappingsToDoubleTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Long> reduceToLong(ConcurrentHashMapV8<K, V> map, ObjectByObjectToLong<? super K, ? super V> transformer, long basis, LongByLongToLong reducer)
		{
			...
			return new MapReduceMappingsToLongTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Integer> reduceToInt(ConcurrentHashMapV8<K, V> map, ObjectByObjectToInt<? super K, ? super V> transformer, int basis, IntByIntToInt reducer)
		{
			...
			return new MapReduceMappingsToIntTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V, U> ForkJoinTask<Void> forEachKey(ConcurrentHashMapV8<K, V> map, Fun<? super K, ? extends U> transformer, Action<U> action)
		{
			...
			return new ForEachTransformedKeyTask<>(map, null, -1, transformer, action);
		}
		...
		public static <K, V, U> ForkJoinTask<U> searchKeys(ConcurrentHashMapV8<K, V> map, Fun<? super K, ? extends U> searchFunction)
		{
			...
			return new SearchKeysTask<>(map, null, -1, searchFunction, new AtomicReference<U>());
		}
		...
		public static <K, V> ForkJoinTask<K> reduceKeys(ConcurrentHashMapV8<K, V> map, BiFun<? super K, ? super K, ? extends K> reducer)
		{
			...
			return new ReduceKeysTask<>(map, null, -1, null, reducer);
		}
		...
		public static <K, V, U> ForkJoinTask<U> reduceKeys(ConcurrentHashMapV8<K, V> map, Fun<? super K, ? extends U> transformer, BiFun<? super U, ? super U, ? extends U> reducer)
		{
			...
			return new MapReduceKeysTask<>(map, null, -1, null, transformer, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Double> reduceKeysToDouble(ConcurrentHashMapV8<K, V> map, ObjectToDouble<? super K> transformer, double basis, DoubleByDoubleToDouble reducer)
		{
			...
			return new MapReduceKeysToDoubleTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Long> reduceKeysToLong(ConcurrentHashMapV8<K, V> map, ObjectToLong<? super K> transformer, long basis, LongByLongToLong reducer)
		{
			...
			return new MapReduceKeysToLongTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Integer> reduceKeysToInt(ConcurrentHashMapV8<K, V> map, ObjectToInt<? super K> transformer, int basis, IntByIntToInt reducer)
		{
			...
			return new MapReduceKeysToIntTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V, U> ForkJoinTask<Void> forEachValue(ConcurrentHashMapV8<K, V> map, Fun<? super V, ? extends U> transformer, Action<U> action)
		{
			...
			return new ForEachTransformedValueTask<>(map, null, -1, transformer, action);
		}
		...
		public static <K, V, U> ForkJoinTask<U> searchValues(ConcurrentHashMapV8<K, V> map, Fun<? super V, ? extends U> searchFunction)
		{
			...
			return new SearchValuesTask<>(map, null, -1, searchFunction, new AtomicReference<U>());
		}
		...
		public static <K, V> ForkJoinTask<V> reduceValues(ConcurrentHashMapV8<K, V> map, BiFun<? super V, ? super V, ? extends V> reducer)
		{
			...
			return new ReduceValuesTask<>(map, null, -1, null, reducer);
		}
		...
		public static <K, V, U> ForkJoinTask<U> reduceValues(ConcurrentHashMapV8<K, V> map, Fun<? super V, ? extends U> transformer, BiFun<? super U, ? super U, ? extends U> reducer)
		{
			...
			return new MapReduceValuesTask<>(map, null, -1, null, transformer, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Double> reduceValuesToDouble(ConcurrentHashMapV8<K, V> map, ObjectToDouble<? super V> transformer, double basis, DoubleByDoubleToDouble reducer)
		{
			...
			return new MapReduceValuesToDoubleTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Long> reduceValuesToLong(ConcurrentHashMapV8<K, V> map, ObjectToLong<? super V> transformer, long basis, LongByLongToLong reducer)
		{
			...
			return new MapReduceValuesToLongTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Integer> reduceValuesToInt(ConcurrentHashMapV8<K, V> map, ObjectToInt<? super V> transformer, int basis, IntByIntToInt reducer)
		{
			...
			return new MapReduceValuesToIntTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V, U> ForkJoinTask<Void> forEachEntry(ConcurrentHashMapV8<K, V> map, Fun<Map.Entry<K, V>, ? extends U> transformer, Action<U> action)
		{
			...
			return new ForEachTransformedEntryTask<>(map, null, -1, transformer, action);
		}
		...
		public static <K, V, U> ForkJoinTask<U> searchEntries(ConcurrentHashMapV8<K, V> map, Fun<Map.Entry<K, V>, ? extends U> searchFunction)
		{
			...
			return new SearchEntriesTask<>(map, null, -1, searchFunction, new AtomicReference<U>());
		}
		...
		public static <K, V> ForkJoinTask<Map.Entry<K, V>> reduceEntries(ConcurrentHashMapV8<K, V> map, BiFun<Map.Entry<K, V>, Map.Entry<K, V>, ? extends Map.Entry<K, V>> reducer)
		{
			...
			return new ReduceEntriesTask<>(map, null, -1, null, reducer);
		}
		...
		public static <K, V, U> ForkJoinTask<U> reduceEntries(ConcurrentHashMapV8<K, V> map, Fun<Map.Entry<K, V>, ? extends U> transformer, BiFun<? super U, ? super U, ? extends U> reducer)
		{
			...
			return new MapReduceEntriesTask<>(map, null, -1, null, transformer, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Double> reduceEntriesToDouble(ConcurrentHashMapV8<K, V> map, ObjectToDouble<Map.Entry<K, V>> transformer, double basis, DoubleByDoubleToDouble reducer)
		{
			...
			return new MapReduceEntriesToDoubleTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Long> reduceEntriesToLong(ConcurrentHashMapV8<K, V> map, ObjectToLong<Map.Entry<K, V>> transformer, long basis, LongByLongToLong reducer)
		{
			...
			return new MapReduceEntriesToLongTask<>(map, null, -1, null, transformer, basis, reducer);
		}
		...
		public static <K, V> ForkJoinTask<Integer> reduceEntriesToInt(ConcurrentHashMapV8<K, V> map, ObjectToInt<Map.Entry<K, V>> transformer, int basis, IntByIntToInt reducer)
		{
			...
			return new MapReduceEntriesToIntTask<>(map, null, -1, null, transformer, basis, reducer);
		}
	}
	...
	static final class ForEachTransformedKeyTask<K, V, U>extends Traverser<K, V, Void>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((action = this.action) != null))
			{
				for (int b; (b = preSplit()) > 0;)
				{
					new ForEachTransformedKeyTask<>(map, this, b, transformer, action).fork();
				}
				U u;
				while (advance() != null)
				{
					u = transformer.apply((K) nextKey);
					if (u != null)
					{
						action.apply(u);
					}
	...
	static final class ForEachTransformedValueTask<K, V, U>extends Traverser<K, V, Void>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((action = this.action) != null))
			{
				for (int b; (b = preSplit()) > 0;)
				{
					new ForEachTransformedValueTask<>(map, this, b, transformer, action).fork();
				}
				V v;
				U u;
				while ((v = advance()) != null)
	...
	static final class ForEachTransformedEntryTask<K, V, U>extends Traverser<K, V, Void>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((action = this.action) != null))
			{
				for (int b; (b = preSplit()) > 0;)
				{
					new ForEachTransformedEntryTask<>(map, this, b, transformer, action).fork();
				}
				V v;
				U u;
				while ((v = advance()) != null)
				{
					u = transformer.apply(entryFor((K) nextKey, v));
					if (u != null)
					{
						action.apply(u);
					}
	...
	static final class ForEachTransformedMappingTask<K, V, U>extends Traverser<K, V, Void>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((action = this.action) != null))
			{
				for (int b; (b = preSplit()) > 0;)
				{
					new ForEachTransformedMappingTask<>(map, this, b, transformer, action).fork();
				}
				V v;
				U u;
				while ((v = advance()) != null)
				{
					u = transformer.apply((K) nextKey, v);
					if (u != null)
					{
						action.apply(u);
					}
		...
		public final void compute()
		{
			...
			if ((searchFunction != null) && ((result = this.result) != null))
			{
				for (int b;;)
				{
					...
					new SearchKeysTask<>(map, this, b, searchFunction, result).fork();
				}
				while (result.get() == null)
				{
					...
					u = searchFunction.apply((K) nextKey);
					if (u != null)
					{
		...
		public final void compute()
		{
			...
			if ((searchFunction != null) && ((result = this.result) != null))
			{
				for (int b;;)
				{
					...
					new SearchValuesTask<>(map, this, b, searchFunction, result).fork();
				}
				while (result.get() == null)
				{
					V v;
	...
	static final class SearchEntriesTask<K, V, U>extends Traverser<K, V, U>
	{
		...
		SearchEntriesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b, Fun<Entry<K, V>, ? extends U> searchFunction, AtomicReference<U> result)
		{
			super(m, p, b);
			this.searchFunction = searchFunction;
			this.result = result;
		...
		public final void compute()
		{
			...
			if ((searchFunction != null) && ((result = this.result) != null))
			{
				for (int b;;)
				{
					...
					new SearchEntriesTask<>(map, this, b, searchFunction, result).fork();
				}
				while (result.get() == null)
				{
					V v;
					...
					u = searchFunction.apply(entryFor((K) nextKey, v));
					if (u != null)
					{
						if (result.compareAndSet(null, u))
						{
		...
		public final void compute()
		{
			...
			if ((searchFunction != null) && ((result = this.result) != null))
			{
				for (int b;;)
				{
					...
					new SearchMappingsTask<>(map, this, b, searchFunction, result).fork();
				}
				while (result.get() == null)
				{
					V v;
					...
					u = searchFunction.apply((K) nextKey, v);
					if (u != null)
					{
						if (result.compareAndSet(null, u))
						{
	...
	static final class ReduceKeysTask<K, V>extends Traverser<K, V, K>
	{
		...
		public final void compute()
		{
			...
			if (reducer != null)
			{
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new ReduceKeysTask<>(map, this, b, rights, reducer)).fork();
				}
				K r = null;
				while (advance() != null)
				{
	...
	static final class ReduceValuesTask<K, V>extends Traverser<K, V, V>
	{
		...
		public final void compute()
		{
			...
			if (reducer != null)
			{
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new ReduceValuesTask<>(map, this, b, rights, reducer)).fork();
				}
				V r = null;
				V v;
				while ((v = advance()) != null)
	...
	static final class ReduceEntriesTask<K, V>extends Traverser<K, V, Map.Entry<K, V>>
	{
		...
		ReduceEntriesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b, ReduceEntriesTask<K, V> nextRight, BiFun<Entry<K, V>, Map.Entry<K, V>, ? extends Map.Entry<K, V>> reducer)
		{
			super(m, p, b);
			this.nextRight = nextRight;
			this.reducer = reducer;
		...
		public final void compute()
		{
			...
			if (reducer != null)
			{
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new ReduceEntriesTask<>(map, this, b, rights, reducer)).fork();
				}
				Map.Entry<K, V> r = null;
				V v;
				while ((v = advance()) != null)
				{
					final Map.Entry<K, V> u = entryFor((K) nextKey, v);
					r = (r == null) ? u : reducer.apply(r, u);
				}
				result = r;
				CountedCompleter<?> c;
	...
	static final class MapReduceKeysTask<K, V, U>extends Traverser<K, V, U>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceKeysTask<>(map, this, b, rights, transformer, reducer)).fork();
				}
				U r = null, u;
				while (advance() != null)
				{
	...
	static final class MapReduceValuesTask<K, V, U>extends Traverser<K, V, U>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceValuesTask<>(map, this, b, rights, transformer, reducer)).fork();
				}
				U r = null, u;
				V v;
				while ((v = advance()) != null)
	...
	static final class MapReduceEntriesTask<K, V, U>extends Traverser<K, V, U>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceEntriesTask<>(map, this, b, rights, transformer, reducer)).fork();
				}
				U r = null, u;
				V v;
				while ((v = advance()) != null)
	...
	static final class MapReduceMappingsTask<K, V, U>extends Traverser<K, V, U>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceMappingsTask<>(map, this, b, rights, transformer, reducer)).fork();
				}
				U r = null, u;
				V v;
				while ((v = advance()) != null)
	...
	static final class MapReduceKeysToDoubleTask<K, V>extends Traverser<K, V, Double>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceKeysToDoubleTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				while (advance() != null)
				{
	...
	static final class MapReduceValuesToDoubleTask<K, V>extends Traverser<K, V, Double>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceValuesToDoubleTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static final class MapReduceEntriesToDoubleTask<K, V>extends Traverser<K, V, Double>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceEntriesToDoubleTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static final class MapReduceMappingsToDoubleTask<K, V>extends Traverser<K, V, Double>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceMappingsToDoubleTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static final class MapReduceKeysToLongTask<K, V>extends Traverser<K, V, Long>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceKeysToLongTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				while (advance() != null)
				{
					r = reducer.apply(r, transformer.apply((K) nextKey));
	...
	static final class MapReduceValuesToLongTask<K, V>extends Traverser<K, V, Long>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceValuesToLongTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static final class MapReduceEntriesToLongTask<K, V>extends Traverser<K, V, Long>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceEntriesToLongTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static final class MapReduceMappingsToLongTask<K, V>extends Traverser<K, V, Long>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceMappingsToLongTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static final class MapReduceKeysToIntTask<K, V>extends Traverser<K, V, Integer>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceKeysToIntTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				while (advance() != null)
				{
					r = reducer.apply(r, transformer.apply((K) nextKey));
	...
	static final class MapReduceValuesToIntTask<K, V>extends Traverser<K, V, Integer>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceValuesToIntTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static final class MapReduceEntriesToIntTask<K, V>extends Traverser<K, V, Integer>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceEntriesToIntTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static final class MapReduceMappingsToIntTask<K, V>extends Traverser<K, V, Integer>
	{
		...
		public final void compute()
		{
			...
			if ((transformer != null) && ((reducer = this.reducer) != null))
			{
				...
				for (int b; (b = preSplit()) > 0;)
				{
					(rights = new MapReduceMappingsToIntTask<>(map, this, b, rights, transformer, r, reducer)).fork();
				}
				V v;
				while ((v = advance()) != null)
				{
	...
	static
	{
		try
		{
			...
			final int scale = U.arrayIndexScale(sc);
			if ((scale & (scale - 1)) != 0)
			{
