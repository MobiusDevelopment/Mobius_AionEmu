 ...
 * CountedCompleter are similar to those of other completion based components (such as {@link java.nio.channels.CompletionHandler}) except that multiple <em>pending</em> completions may be necessary to trigger the {@link #onCompletion} action, not just one. Unless initialized otherwise, the
 * {@link #getPendingCount pending count} starts at zero, but may be (atomically) changed using methods {@link #setPendingCount}, {@link #addToPendingCount}, and {@link #compareAndSetPendingCount}. Upon invocation of {@link #tryComplete}, if the pending action count is nonzero, it is decremented;
 * otherwise, the completion action is performed, and if this completer itself has a completer, the process is continued with its completer. As is the case with related synchronization components such as {@link java.util.concurrent.Phaser Phaser} and {@link java.util.concurrent.Semaphore Semaphore},
 * these methods affect only internal counts; they do not establish any further internal bookkeeping. In particular, the identities of pending tasks are not maintained. As illustrated below, you can create subclasses that do record some or all pending tasks or their results when needed. As
 * illustrated below, utility methods supporting customization of completion traversals are also provided. However, because CountedCompleters provide only basic synchronization mechanisms, it may be useful to create further abstract subclasses that maintain linkages, fields, and additional support
 * methods appropriate for a set of related usages.
 * <p/>
 * <p/>
 * A concrete CountedCompleter class must define method {@link #compute}, that should in most cases (as illustrated below), invoke {@code tryComplete()} once before returning. The class may also optionally override method {@link #onCompletion} to perform an action upon normal completion, and method
 * {@link #onExceptionalCompletion} to perform an action upon any exception.
 * <p/>
 * <p/>
 * CountedCompleters most often do not bear results, in which case they are normally declared as {@code CountedCompleter<Void>}, and will always return {@code null} as a result value. In other cases, you should override method {@link #getRawResult} to provide a result from {@code join(), invoke()},
 * and related methods. In general, this method should return the value of a field (or a function of one or more fields) of the CountedCompleter object that holds the result upon completion. Method {@link #setRawResult} by default plays no role in CountedCompleters. It is possible, but rarely
 * applicable, to override this method to maintain other objects or fields holding result data.
 * <p/>
 * <p/>
 * A CountedCompleter that does not itself have a completer (i.e., one for which {@link #getCompleter} returns {@code null}) can be used as a regular ForkJoinTask with this added functionality. However, any completer that in turn has another completer serves only as an internal helper for other
 * computations, so its own task status (as reported in methods such as {@link ForkJoinTask#isDone}) is arbitrary; this status changes only upon explicit invocations of null {@link #complete}, {@link ForkJoinTask#cancel}, {@link ForkJoinTask#completeExceptionally} or upon exceptional completion of
 * method {@code compute}. Upon any exceptional completion, the exception may be relayed to a task's completer (and its completer, and so on), if one exists and it has not otherwise already completed. Similarly, cancelling an internal CountedCompleter has only a local effect on that completer, so is
 * not often useful.
 * <p/>
 * <p/>
 * <b>Sample Usages.</b>
 * <p/>
 * <p/>
 * <b>Parallel recursive decomposition.</b> CountedCompleters may be arranged in trees similar to those often used with {@link RecursiveAction}s, although the constructions involved in setting them up typically vary. Here, the completer of each task is its parent in the computation tree. Even though
 * they entail a bit more bookkeeping, CountedCompleters may be better choices when applying a possibly time-consuming operation (that cannot be further subdivided) to each element of an array or collection; especially when the operation takes a significantly different amount of time to complete for
 * some elements than others, either because of intrinsic variation (for example I/O) or auxiliary effects such as garbage collection. Because CountedCompleters provide their own continuations, other threads need not block waiting to perform them.
 * <p/>
 * <p/>
 * For example, here is an initial version of a class that uses divide-by-two recursive decomposition to divide work into single pieces (leaf tasks). Even when work is split into individual calls, tree-based techniques are usually preferable to directly forking leaf tasks, because they reduce
 ...
 * <p/>
 * public void compute() { // version 1 if (hi - lo >= 2) { int mid = (lo + hi) >>> 1; setPendingCount(2); // must set pending count before fork new ForEach(this, array, op, mid, hi).fork(); // right child new ForEach(this, array, op, lo, mid).fork(); // left child } else if (hi > lo)
 ...
 * <p/>
 * <p/>
 * <b>Searching.</b> A tree of CountedCompleters can search for a value or property in different parts of a data structure, and report a result in an {@link java.util.concurrent.atomic.AtomicReference AtomicReference} as soon as one is found. The others can poll the result to avoid unnecessary work.
 ...
 * </pre>
 * <p/>
 * In this example, as well as others in which tasks have no other effects except to compareAndSet a common result, the trailing unconditional invocation of {@code tryComplete} could be made conditional ({@code if (result.get() == null) tryComplete();}) because no further bookkeeping is required to
 * manage completions once the root task completes.
 * <p/>
 * <p/>
 * <b>Recording subtasks.</b> CountedCompleter tasks that combine results of multiple subtasks usually need to access these results in method {@link #onCompletion}. As illustrated in the following class (that performs a simplified form of map-reduce where mappings and reductions are all of type
 ...
 * reducer.apply(child.result, sib.result); } } public E getRawResult() { return result; }
 * <p/>
 ...
 * </pre>
 * <p/>
 * Here, method {@code onCompletion} takes a form common to many completion designs that combine results. This callback-style method is triggered once per task, in either of the two different contexts in which the pending count is, or becomes, zero: (1) by a task itself, if its pending count is zero
 * upon invocation of {@code
 * tryComplete}, or (2) by any of its subtasks when they complete and decrement the pending count to zero. The {@code caller} argument distinguishes cases. Most often, when the caller is {@code this}, no action is necessary. Otherwise the caller argument can be used (usually via a cast) to supply a
 * value (and/or links to other values) to be combined. Assuming proper use of pending counts, the actions inside {@code onCompletion} occur (once) upon completion of a task and its subtasks. No additional synchronization is required within this method to ensure thread safety of accesses to fields
 * of this task or other completed tasks.
 * <p/>
 * <p/>
...
public abstract class CountedCompleter<T>extends ForkJoinTask<T>
{
	...
	/**
	 * Performs an action when method {@link #tryComplete} is invoked and the pending count is zero, or when the unconditional method {@link #complete} is invoked. By default, this method does nothing. You can distinguish cases by checking the identity of the given caller argument. If not equal to
	 * {@code
	 * this}, then it is typically a subtask that may contain results (and/or links to other results) to combine.
	...
	/**
	 * Performs an action when method {@link #completeExceptionally} is invoked or method {@link #compute} throws an exception, and this task has not otherwise already completed normally. On entry to this method, this task {@link ForkJoinTask#isCompletedAbnormally}. The return value of this method
	 * controls further propagation: If {@code true} and this task has a completer, then this completer is also completed exceptionally. The default implementation of this method does nothing except return {@code true}.
	 * @param ex the exception
	 * @param caller the task invoking this method (which may be this task itself)
	 * @return true if this exception should be propagated to this task's completer, if one exists
	 */
	...
	/**
	 * If the pending count is nonzero, decrements the count; otherwise invokes {@link #onCompletion} and then similarly tries to complete this task's completer, if one exists, else marks this task as complete.
	 */
	...
	/**
	 * Equivalent to {@link #tryComplete} but does not invoke {@link #onCompletion} along the completion path: If the pending count is nonzero, decrements the count; otherwise, similarly tries to complete this task's completer, if one exists, else marks this task as complete. This method may be
	 * useful in cases where {@code onCompletion} should not, or need not, be invoked for each completer in a computation.
	 */
	...
	/**
	 * Regardless of pending count, invokes {@link #onCompletion}, marks this task as complete and further triggers {@link #tryComplete} on this task's completer, if one exists. The given rawResult is used as an argument to {@link #setRawResult} before invoking {@link #onCompletion} or marking this
	 * task as complete; its value is meaningful only for classes overriding {@code setRawResult}.
	 * <p/>
	 * <p/>
	 * This method may be useful when forcing completion as soon as any one (versus all) of several subtask results are obtained. However, in the common (and recommended) case in which {@code
	 * setRawResult} is not overridden, this effect can be obtained more simply using {@code quietlyCompleteRoot();}.
	...
	/**
	 * If this task's pending count is zero, returns this task; otherwise decrements its pending count and returns {@code
	 * null}. This method is designed to be used with {@link #nextComplete} in completion traversal loops.
