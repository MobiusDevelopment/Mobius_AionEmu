...
public class GlobalCallbackEnhancer extends CallbackClassFileTransformer
{
	...
	protected void enhanceMethod(CtMethod method) throws CannotCompileException, NotFoundException, ClassNotFoundException
	{
		...
		final CtClass listenerClazz = cp.get(((GlobalCallback) method.getAnnotation(GlobalCallback.class)).value().getName());
		
		final boolean isStatic = Modifier.isStatic(method.getModifiers());
		final String listenerFieldName = "$$$" + (isStatic ? "Static" : "") + listenerClazz.getSimpleName();
	...
	/**
	 * Checks if method is enhanceable. It should be marked with {@link com.aionemu.commons.callbacks.metadata.GlobalCallback} annotation, be not native and not abstract
