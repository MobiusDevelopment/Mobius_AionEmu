...
public class ObjectCallbackEnhancer extends CallbackClassFileTransformer
{
	...
	protected byte[] transformClass(ClassLoader loader, byte[] clazzBytes) throws Exception
	{
		...
		final CtClass clazz = cp.makeClass(new ByteArrayInputStream(clazzBytes));
		...
		{
			final CtClass eo = cp.get(EnhancedObject.class.getName());
			for (final CtClass i : clazz.getInterfaces())
			{
				if (i.equals(eo))
				{
	...
	{
		final ClassPool cp = method.getDeclaringClass().getClassPool();
		
		method.addLocalVariable("___cbr", cp.get(CallbackResult.class.getName()));
		
		final CtClass listenerClazz = cp.get(((ObjectCallback) method.getAnnotation(ObjectCallback.class)).value().getName());
		
		final String listenerFieldName = "$$$" + listenerClazz.getSimpleName();
	...
	{
		final ClassPool cp = clazz.getClassPool();
	...
	private void writeEnhancedOBjectFields(CtClass clazz) throws CannotCompileException, NotFoundException
	{
		...
		final CtField cbField = new CtField(cp.get(Map.class.getName()), FIELD_NAME_CALLBACKS, clazz);
		...
		final CtField cblField = new CtField(cp.get(ReentrantReadWriteLock.class.getName()), FIELD_NAME_CALLBACKS_LOCK, clazz);
	...
	private void writeEnhancedObjectMethods(CtClass clazz) throws NotFoundException, CannotCompileException
	{
		...
		final CtClass callbackClass = cp.get(Callback.class.getName());
		final CtClass mapClass = cp.get(Map.class.getName());
		final CtClass reentrantReadWriteLockClass = cp.get(ReentrantReadWriteLock.class.getName());
		
		CtMethod method = new CtMethod(CtClass.voidType, "addCallback", new CtClass[]
		{
			callbackClass
		}, clazz);
		...
		method = new CtMethod(CtClass.voidType, "removeCallback", new CtClass[]
		{
			callbackClass
		}, clazz);
		method.setModifiers(java.lang.reflect.Modifier.PUBLIC);
		...
		method = new CtMethod(CtClass.voidType, "setCallbacks", new CtClass[]
		{
			mapClass
		}, clazz);
		method.setModifiers(java.lang.reflect.Modifier.PUBLIC);
	...
	/**
	 * Checks if method is enhanceable. It should be marked with {@link com.aionemu.commons.callbacks.metadata.ObjectCallback} annotation, be not native and not abstract
