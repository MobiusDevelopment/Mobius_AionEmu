...
public class ObjectCallbackHelper
{
	...
	public static void removeCallback(Callback callback, EnhancedObject object)
	{
		try
		{
			...
			final Map<Class<? extends Callback<?>>, List<Callback<?>>> cbMap = object.getCallbacks();
			if (GenericValidator.isBlankOrNull(cbMap))
			{
				return;
			}
			
			final List<Callback<?>> list = cbMap.get(callback.getBaseClass());
	...
	{
		final Map<Class<? extends Callback<?>>, List<Callback<?>>> cbMap = obj.getCallbacks();
		if (GenericValidator.isBlankOrNull(cbMap))
		{
