 ...
 * Briefly speaking, if you will try to remove/add a listener from another listener - the current invocation won't be affected, current implementation allocates all listeners that are going to be invoked before execution.<br>
 * <br>
 * {@link Callback#beforeCall(Object, Object[])} and {@link Callback#afterCall(Object, Object[], Object)} are treated as separate invocations, so adding/removing listener in beforeCall will affect afterCall.
 * @author SoulKeeper
 */
public interface EnhancedObject
{
	 ...
	 * <br>
	 * <b><font color="red"> Iteration over this map is not thread-safe, please make sure that {@link #getCallbackLock()} is locked in read mode to read.<br>
	 * <br>
	 ...
	 */
	void setCallbacks(Map<Class<? extends Callback<?>>, List<Callback<?>>> callbacks);
