...
import com.aionemu.loginserver.network.gs.serverpackets.SM_BAN_RESPONSE;
...
public class CM_BAN extends GsClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (((type == 1) || (type == 3)) && (accountId != 0))
		{
			...
			{
				final AccountTime accountTime = DAOManager.getDAO(AccountTimeDAO.class).getAccountTime(accountId);
				accountTime.setPenaltyEnd(newTime);
		...
		if ((type == 2) || (type == 3))
		{
			...
			{
				final String newip = DAOManager.getDAO(AccountDAO.class).getLastIp(accountId);
				if (!newip.isEmpty())
				{
					ip = newip;
				}
			...
			if (!ip.isEmpty())
			{
				...
				if (time >= 0) // Ban
				{
					final Timestamp newTime = time != 0 ? new Timestamp(System.currentTimeMillis() + (time * 60000)) : null;
