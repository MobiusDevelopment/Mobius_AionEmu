...
public class PlayerTransferService
{
	...
	protected void verifyNewTasks()
	{
		...
		for (PlayerTransferTask task : tasksNew)
		{
			...
			final GameServerInfo targetServer = GameServerTable.getGameServerInfo(task.targetServerId);
	...
	{
		final PlayerTransferTask task = tasks.get(taskId);
		final GameServerInfo targetServer = GameServerTable.getGameServerInfo(task.targetServerId);
		if ((targetServer == null) || (targetServer.getConnection() == null))
		{
		...
		final Account account = AccountController.loadAccount(task.targetAccountId);
		final Account saccount = AccountController.loadAccount(task.sourceAccountId);
		
		final PlayerTransferRequest request = new PlayerTransferRequest(PlayerTransferStatus.STEP1);
	...
	{
		final PlayerTransferTask task = tasks.remove(taskId);
	...
	{
		final PlayerTransferRequest request = transfers.remove(taskId);
		final PlayerTransferTask task = tasks.remove(taskId);
		task.status = PlayerTransferTask.STATUS_ERROR;
		task.comment = reason;
		dao.update(task);
		final GameServerInfo targetServer = GameServerTable.getGameServerInfo(request.targetServerId);
		if ((targetServer == null) || (targetServer.getConnection() == null))
		{
	...
	public void onOk(int taskId, int playerId)
	{
		...
		dao.update(task);
		final GameServerInfo sourceServer = GameServerTable.getGameServerInfo(request.serverId);
