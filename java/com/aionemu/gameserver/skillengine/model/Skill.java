...
public class Skill
{
	...
	{
		effectedList = new ArrayList<>();
		conditionChangeListener = new StartMovingListener();
		this.firstTarget = firstTarget;
		skillLevel = skillLvl;
		skillStackLvl = skillTemplate.getLvl();
	...
	public boolean canUseSkill()
	{
		...
		if (effector instanceof Player)
		{
			...
			{
				final long time = player.getLastCounterSkill(skillTemplate.getCounterSkill());
	...
	private boolean useSkill(boolean checkAnimation, boolean checkproperties)
	{
		...
		if (effector instanceof Player)
		{
			...
			{
				MotionLoggingService.getInstance().logTime((Player) effector, getSkillTemplate(), getHitTime(), MathUtil.getDistance(effector, firstTarget));
			}
		}
	...
	private boolean checkAnimationTime()
	{
		...
		final Motion motion = getSkillTemplate().getMotion();
		...
		final MotionTime motionTime = DataManager.MOTION_DATA.getMotionTime(motion.getName());
		...
		final WeaponTypeWrapper weapons = new WeaponTypeWrapper(player.getEquipment().getMainHandWeaponType(), player.getEquipment().getOffHandWeaponType());
		...
		final int finalTime = Math.round(serverTime);
		if (motion.getInstantSkill() && (hitTime == 0))
		{
	...
	{
		final int penaltySkill = skillTemplate.getPenaltySkillId();
		if (penaltySkill == 0)
		{
			return;
		}
	...
	private void endCast()
	{
		...
		{
			final Item item = ((Player) effector).getInventory().getItemByObjId(itemObjectId);
			if (item == null)
			{
				return;
			}
		...
		if (skillTemplate.getEffects() != null)
		{
			...
			if (effectedList.isEmpty() && isPointPointSkill())
			{
				final Effect effect = new Effect(this, null, 0, itemTemplate);
				effect.initialize();
				final int worldId = effector.getWorldId();
				final int instanceId = effector.getInstanceId();
				effect.setWorldPosition(worldId, instanceId, x, y, z);
		...
		{
			final int chainProb = skillTemplate.getChainSkillProb();
			if ((chainCategory != null) && !blockedChain)
			{
				chainSuccess = Rnd.get(90) < chainProb;
			}
		}
		else
		{
	...
	{
		final Conditions skillConditions = skillTemplate.getUseconditions();
		return skillConditions != null ? skillConditions.validate(this) : true;
	}
