...
public class KillInWorldData extends XMLQuest
{
	...
	public void register(QuestEngine questEngine)
	{
		...
		{
			final Iterator<WorldMapTemplate> itr = DataManager.WORLD_MAPS_DATA.iterator();
			worldIds.clear();
			while (itr.hasNext())
			{
				final WorldMapTemplate template = itr.next();
				worldIds.add(template.getMapId());
			}
		}
		final KillInWorld template = new KillInWorld(id, endNpcIds, startNpcIds, worldIds, amount, invasionWorld);
