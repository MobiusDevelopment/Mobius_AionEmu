...
public class HousingBidService extends AbstractCronTask
{
	...
	public boolean isBiddingAllowed()
	{
		...
		final DateTime auctionEnd = new DateTime(((long) getRunTime() + (timeProlonged * 60)) * 1000);
	...
	{
		final DateTime now = DateTime.now();
		final DateTime registerEnd = new DateTime(registerDateExpr.getTimeAfter(now.toDate()));
		final DateTime auctionEnd = new DateTime(((long) getRunTime() + (timeProlonged * 60)) * 1000);
		if (((now.getDayOfWeek() == registerEnd.getDayOfWeek()) && (now.getHourOfDay() >= registerEnd.getHourOfDay())) || ((now.getDayOfWeek() == auctionEnd.getDayOfWeek()) && (now.getHourOfDay() <= auctionEnd.getHourOfDay())))
		{
			return false;
		}
		return true;
