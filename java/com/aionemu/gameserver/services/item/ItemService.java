...
public class ItemService
{
	...
	{
		final Storage inventory = player.getInventory();
		final ItemCustomSetTeamplate itemCustomSet = DataManager.ITEM_CUSTOM_SET_DATA.getCustomTemplate(itemTemplate.getItemCustomSet());
	...
	{
		final Collection<Integer> idIterator = Collections2.transform(items, AionObject.OBJECT_TO_ID_TRANSFORMER);
	...
	public static Item newItem(int itemId, long count, String crafterName, int ownerId, long tempItemTime, int tempTradeTime)
	{
		...
		final int maxStackCount = (int) itemTemplate.getMaxStackCount();
		...
		final Item temp = new Item(IDFactory.getInstance().nextId(), itemTemplate, count, false, 0);
