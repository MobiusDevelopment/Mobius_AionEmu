...
import com.aionemu.gameserver.network.aion.serverpackets.SM_PLAYER_ESSENCE;
...
public class CreativityEssenceService
{
	...
	{
		final float current = player.getCommonData().getExpShown();
		final float total = player.getCommonData().getExpNeed();
		final float percent = getPercentage(current, total);
		final int step = player.getCPStep();
