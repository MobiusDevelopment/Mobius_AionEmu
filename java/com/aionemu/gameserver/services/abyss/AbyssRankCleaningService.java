...
public class AbyssRankCleaningService
{
	...
	private void runCleaning()
	{
		...
		final int periodInDays = CleaningConfig.ABYSS_CLEANING_PERIOD;
	...
	private void runAbyssRankingCleaning()
	{
		...
		for (final AbyssRankingResult result : rankingsElyos)
		{
			...
			final Timestamp t = p.getCommonData().getLastOnline();
			final boolean isOutOfDate = (t.getTime() / 1000) >= 43200 ? true : false; // Every 30 Days
			if (isOutOfDate)
			{
				ToArray.add(p);
			}
		...
		for (final AbyssRankingResult result : rankingsAsmos)
		{
			...
			final Timestamp t = p.getCommonData().getLastOnline();
			final boolean isOutOfDate = (t.getTime() / 1000) >= 43200 ? true : false;
			if (isOutOfDate)
			{
				ToArray.add(p);
			}
