...
public class LadderService
{
	...
	private final List<AionObject> eventQueueList = new ArrayList<>();
	private final List<AionObject> normalQueueList = new ArrayList<>();
	private final List<AionObject> soloQueueList = new ArrayList<>();
	...
	private Battleground getRandomBg(boolean teamOnly)
	{
		...
		{
			TwoTeamBg.class,
			TwoTeamSmallBg.class
		};
		if (!teamOnly)
		{
			bgs = new Class<?>[]
			{
				SoloSurvivorBg.class,
				SoloSurvivorBg.class,
				SoloSurvivorBg.class,
				SoloSurvivorBg.class,
				DeathmatchBg.class,
	...
	private void HandleNormalQueue(BattlegroundEvent event)
	{
		...
		while (iterations++ < 40)
		{
			final Battleground bg = getRandomBg(normalTeamBased);
			bg.setTeamBased(normalTeamBased);
			if (normalTeamBased)
			{
				final List<List<Player>> usedGroups = new ArrayList<>();
				final List<List<Player>> groups = new ArrayList<>();
				final List<Integer> participants = new ArrayList<>();
			...
			if (bg.hasPlayers())
			{
				final Integer bgId = registerBg(bg);
	...
	private void HandleEventQueue()
	{
		...
		final Class<?> eventBgClass = eventBg.getClass();
		eventQueueList.clear();
		int iterations = 0;
		while (iterations++ < 40)
		{
	...
	private void HandleSoloQueue()
	{
		...
		{
			final Battleground bg = new SoloSurvivorBg();
			final List<Integer> participants = new ArrayList<>(2);
	...
	public void onBgEnd(Battleground bg)
	{
		...
		{
			final Event event = normalBgMap.remove(bg.getBgId());
	...
	{
		final Integer bgId = getNextAvailableBgId();
		bg.setBgId(bgId);
		bgMap.put(bgId, bg);
		return bgId;
	}
	...
	{
		final LegionEmblem emblem = new LegionEmblem();
	...
	{
		final List<Integer> warrior = new ArrayList<>();
		...
		{
			final int total = warrior.size() + scout.size() + mage.size() + cleric.size();
			final float ratioW = (warrior.size() * 100) / total;
			final float ratioS = (scout.size() * 100) / total;
			final float ratioM = (mage.size() * 100) / total;
			final float ratioP = (cleric.size() * 100) / total;
			if (!warrior.isEmpty())
			{
