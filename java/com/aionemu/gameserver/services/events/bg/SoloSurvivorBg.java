...
public class SoloSurvivorBg extends Battleground
{
	...
	public void startMatch()
	{
		...
		ThreadPoolManager.getInstance().schedule(() -> openStaticDoors(), 25 * 1000);
		super.setExpireTask(ThreadPoolManager.getInstance().schedule(() -> endSoloSurvivorMatch(true), getMatchLength() * 1000));
	...
	public boolean createTournament(List<List<Player>> players)
	{
		...
		maxRounds = 5;
		matchLength = 145;
		description = "You must kill your opponent to win. You have " + maxRounds + " round. The player who gets the most kill's, win the match !!!";
		startMatch();
		return true;
	}
	...
	void endSoloSurvivorMatch(boolean isDraw)
	{
		if (endCalled)
		{
			return;
		...
		final Player roundWinner = getRoundWinner();
