...
public abstract class Battleground
{
	...
	protected Map<Integer, WorldPosition> previousLocations = new FastMap<>();
	...
	{
		final int avgCount = (int) Math.floor(playerCount / getTeamCount());
		if (isTeamBased())
		{
			return avgCount;
		}
	...
	{
		final int size = getRandomSize(players.size());
		...
		{
			final int objId = players.remove(0);
			final Player pl = World.getInstance().findPlayer(objId);
			if (pl == null)
			{
				continue;
			}
	...
	{
		final int groupSize = getRandomSize(players.size());
		...
		{
			final List<Player> groupPlayers = new ArrayList<>();
			...
			final PlayerGroup group = PlayerGroupService.createGroup(groupPlayers.get(0));
	...
	{
		final int allianceSize = getRandomSize(players.size());
		...
		{
			final List<Player> alliancePlayers = new ArrayList<>();
			...
			final PlayerAlliance alliance = new PlayerAlliance(new PlayerAllianceMember(alliancePlayers.get(0)), TeamType.ALLIANCE);
	...
	protected boolean createGroups(List<List<Player>> teams)
	{
		...
		{
			final List<Player> players = teams.remove(0);
			while (players.size() > groupSize)
			{
			...
			final PlayerGroup group = PlayerGroupService.createGroup(players.get(0));
	...
	protected boolean createAlliances(List<List<Player>> teams)
	{
		...
		while (getAlliances().size() < getTeamCount())
		{
			...
			final PlayerAlliance alliance = new PlayerAlliance(new PlayerAllianceMember(players.get(0)), TeamType.ALLIANCE);
	...
	protected void returnToPreviousLocation(Player player)
	{
		...
		endTimer(player);
		final WorldPosition previousPos = previousLocations.get(player.getObjectId());
		if (previousPos == null)
		{
	...
	protected void scheduleAnnouncement(Player player, String msg, int delay)
	{
		...
		{
			scheduleAnnouncement(player, "1vs1", msg, delay);
		}
		else if (player.getBattleground() instanceof DeathmatchBg)
		{
			scheduleAnnouncement(player, "DM", msg, delay);
		}
		else
		{
			scheduleAnnouncement(player, "BG", msg, delay);
		}
	}
	...
	protected void onLeaveDefault(Player player, boolean isLogout, boolean isAfk)
	{
		...
		{
			if (!isDone && !player.getController().isInShutdownProgress())
			{
	...
	public void onSpectatorJoin(Player spectator)
	{
		...
		final SpawnPosition pos = getSpawnPositions().get(Rnd.get(getSpawnPositions().size()));
	...
	public void reconnectPlayer(Player player)
	{
		...
		final String msg = player.getName() + " is back in <Battleground>!";
	...
	protected WorldMapInstance createInstance()
	{
		...
		map = maps.get(Rnd.get(maps.size()));
		mapId = map.getMapId();
		final WorldMapInstance instance = InstanceService.getNextBgInstance(getMapId());
		if (instance != null)
		{
