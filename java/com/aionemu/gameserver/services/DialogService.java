...
public class DialogService
{
	...
	public static void onDialogSelect(int dialogId, Player player, Npc npc, int questId, int extendedRewardIndex)
	{
		...
		final int titleId = npc.getObjectTemplate().getTitleId();
		switch (dialogId)
		{
			case 2:
			{
				...
				final TradeListTemplate tradeListTemplate = DataManager.TRADE_LIST_DATA.getTradeListTemplate(npc.getNpcId());
			...
			case 4:
			{
				...
				switch (titleId)
				{
					...
					case 370503: // <Gogorunerk Solution,INC>
					{
						...
						{
							final QuestState qs = player.getQuestStateList().getQuestState(2900); // No Escaping Destiny.
							if ((qs == null) || (qs.getStatus() != QuestStatus.COMPLETE))
							{
			...
			{ // 4.3
				...
				{
					final boolean result = player.getResponseRequester().putRequest(SM_QUESTION_WINDOW.STR_ASK_RECOVER_EXPERIENCE, responseHandler);
					if (result)
					{
			...
			case 20010:
			{
				if (questId == 0)
				{
					final TeleporterTemplate template = DataManager.TELEPORTER_DATA.getTeleporterTemplateByNpcId(npc.getNpcId());
					final PortalPath portalPath = DataManager.PORTAL2_DATA.getPortalDialog(npc.getNpcId(), dialogId, player.getRace());
					if (portalPath != null)
					{
					...
					{
						final TeleportLocation loc = template.getTeleLocIdData().getTelelocations().get(0);
						if (loc != null)
						{
