...
public final class QuestService
{
	...
	private static List<QuestItems> getRewardItems(QuestEnv env, QuestTemplate template, boolean extended, int reward)
	{
		...
		if ((dialogId != 23) && (dialogId != 0) && !extended)
		{
			...
			if ((isLastRepeat && template.isUseSingleClassReward()) || template.isUseRepeatedClassReward())
			{
				...
				final int selRewIndex = dialogId - 8;
				switch (playerClass)
				{
					case ASSASSIN:
					{
	...
	public static boolean startQuest(QuestEnv env, QuestStatus status, boolean warn)
	{
		...
		// TO DO LATER
		/*
		 * if (template.getTargetType() == QuestTargetType.FORCE || template.getTargetType() == QuestTargetType.UNION) { PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_UNION_YOU_ARE_NOT_UNION_MEMBER); return true; }
		 */
		if (template.getNpcFactionId() != 0)
		{
			final NpcFaction faction = player.getNpcFactions().getNpcFactionById(template.getNpcFactionId());
	...
	{
		final DropItem item = new DropItem(new Drop(drop.getItemId(), 1, 1, drop.getChance(), false, false));
	...
	/*
	 * public static boolean bountyReward(Player player, int questId) { QuestTemplate template = questsData.getQuestById(questId); if (template == null) { return false; } if (!template.isBountyReward()) { return false; } QuestState qs = player.getQuestStateList().getQuestState(questId); if (qs ==
	 * null) { return false; } if (qs.getStatus() == QuestStatus.COMPLETE || qs.getStatus() == QuestStatus.LOCKED) { return false; } QuestEnv env = new QuestEnv(player, player, questId, QuestDialog.AUTO_REWARD.id()); finishQuest(env); player.getController().updateNearbyQuests(); return true; }
	 */
