...
public class DecomposeAction extends AbstractItemAction
{
	...
	{
		final int decomposeLevel = parentItem.getItemTemplate().getRequiredLevel(player.getCommonData().getPlayerClass());
						...
						else if (selectedCollection.getRandomItems().size() > 0)
						{
							for (final RandomItem randomItem : selectedCollection.getRandomItems())
							{
								...
								if (randomType != null)
								{
									int randomId = 0;
									int i = 0;
									final int itemLvl = parentItem.getItemTemplate().getLevel();
									switch (randomItem.getType())
									{
										...
										case MANASTONE_LEGEND_GRADE_65:
											...
											final List<ItemTemplate> stones = manastones.get(randomType.equals(RandomType.MANASTONE) ? itemLvl : randomType.getLevel());
											if (stones == null)
											{
											...
											if (!randomType.equals(RandomType.MANASTONE))
											{
												...
												final List<ItemTemplate> selectedStones = select(stones, having(on(ItemTemplate.class).getItemQuality(), equalTo(itemQuality)));
												randomId = selectedStones.get(Rnd.get(selectedStones.size())).getTemplateId();
											}
											else
											{
												final List<ItemTemplate> selectedStones = select(stones, having(on(ItemTemplate.class).getItemQuality(), not(equalTo(ItemQuality.LEGEND))));
												randomId = selectedStones.get(Rnd.get(selectedStones.size())).getTemplateId();
											}
