...
import com.aionemu.gameserver.network.ls.serverpackets.SM_PREMIUM_CONTROL;
...
public class InGameShopEn
{
	...
	private static Map<Integer, Long> lastUsage = new FastMap<>();
	...
	{
		final byte max = 6;
		final TreeMap<Integer, Integer> map = new TreeMap<>(new DescFilter());
		if (!items.containsKey(category))
		{
	...
	public void acceptRequest(Player player, int itemObjId)
	{
		...
		final IGItem item = getInstance().getIGItem(itemObjId);
		...
		lastRequestId++;
		final IGRequest request = new IGRequest(lastRequestId, player.getObjectId(), itemObjId);
	...
	public void sendRequest(Player player, String receiver, String message, int itemObjId)
	{
		...
		final PlayerCommonData recipientCommonData = DAOManager.getDAO(PlayerDAO.class).loadPlayerCommonDataByName(receiver);
		...
		final IGItem item = getIGItem(itemObjId);
		lastRequestId++;
		final IGRequest request = new IGRequest(lastRequestId, player.getObjectId(), receiver, message, itemObjId);
		request.accountId = player.getClientConnection().getAccount().getId();
		if (LoginServer.getInstance().sendPacket(new SM_PREMIUM_CONTROL(request, item.getItemPrice())))
		{
			activeRequests.add(request);
	...
	public void addToll(Player player, long cnt)
	{
		...
		{
			lastRequestId++;
			final IGRequest request = new IGRequest(lastRequestId, player.getObjectId(), 0);
			request.accountId = player.getClientConnection().getAccount().getId();
	...
	public void finishRequest(int requestId, int result, long toll, long luna)
	{
		for (IGRequest request : activeRequests)
		{
			...
			{
				final Player player = World.getInstance().findPlayer(request.playerId);
				if (player != null)
				{
					if (result == 1)
					{
					...
					{
						final IGItem item = getIGItem(request.itemObjId);
						if (item == null)
						{
							PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_MSG_INGAMESHOP_ERROR);
						...
						// long resultCol = player.getPlayerAccount().getToll() - toll;
