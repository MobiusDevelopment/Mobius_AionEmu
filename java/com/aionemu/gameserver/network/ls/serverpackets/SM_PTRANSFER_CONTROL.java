		...
		switch (type)
		{
			...
			case CHARACTER_INFORMATION:
			{
				...
				for (Item item : inv)
				{
					...
					final GodStone stone = item.getGodStone();
				...
				for (PetCommonData pet : pets)
				{
					...
					final long birthday = pet.getBirthdayTimestamp() == null ? 0 : pet.getBirthdayTimestamp().getTime();
					writeQ(birthday);
					writeS(pet.getName());
				}
				...
				final PlayerSettings ps = player.getPlayerSettings();
