...
public class CM_EMOTION extends AionClientPacket
{
	...
	protected void runImpl()
	{
		...
		switch (emotionType)
		{
			...
			case START_SPRINT:
			{
				...
				if (!player.isInPlayerMode(PlayerMode.RIDE) || (player.getLifeStats().getCurrentFp() < Fp) || player.isInState(CreatureState.FLYING) || !player.ride.canSprint())
				{
					return;
				}
	...
	{
		final int target = player.getTarget() == null ? 0 : player.getTarget().getObjectId();
