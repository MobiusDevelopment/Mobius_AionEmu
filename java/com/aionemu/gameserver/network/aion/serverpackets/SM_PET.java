			...
			case 0:
				...
				for (final PetCommonData petCommonData : pets)
				{
					...
					if (petTemplate.ContainsFunction(PetFunctionType.CHEER))
					{
						...
						final short cheer = (short) petTemplate.getPetFunction(PetFunctionType.CHEER).getId();
						writeH(cheer);
						specialtyCount++;
					}
					if (petTemplate.ContainsFunction(PetFunctionType.MERCHAND))
					{
						...
						final short merchant = (short) petTemplate.getPetFunction(PetFunctionType.MERCHAND).getId();
						writeH(merchant);
						writeC(0x00);
						specialtyCount++;
					}
