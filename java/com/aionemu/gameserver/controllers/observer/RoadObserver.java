...
public class RoadObserver extends ActionObserver
{
	...
	public void moved()
	{
		...
		{
			final Point3D intersectionPoint = road.getPlane().intersection(oldPosition, newPosition);
			if (intersectionPoint != null)
			{
				final double distance = Math.abs(road.getPlane().getCenter().distance(intersectionPoint));
		...
		if (passedThrough)
		{
			final RoadExit exit = road.getTemplate().getRoadExit();
			
			final WorldType type = road.getWorldType();
