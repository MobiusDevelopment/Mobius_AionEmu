...
/**
 * <code>Matrix4f</code> defines and maintains a 4x4 matrix in row major order. This matrix is intended for use in a translation and rotational capacity. It provides convenience methods for creating the matrix from a multitude of sources.
 * <p/>
 * Matrices are stored assuming column vectors on the right, with the translation in the rightmost column. Element numbering is row,column, so m03 is the zeroth row, third column, which is the "x" translation part. This means that the implicit storage order is column major. However, the get() and
...
public final class Matrix4f implements Cloneable
{
	...
	/**
	 * Constructor instantiates a new <code>Matrix</code> that is set to the provided matrix. This constructor copies a given Matrix. If the provided matrix is null, the constructor sets the matrix to the identity.
	 ...
	 * @param i the column to retrieve. Must be between 0 and 3.
	 * @param store the float array to store the result in. if null, a new one is created.
	 * @return the column specified by the index.
	 */
	...
	/**
	 * <code>toFloatBuffer</code> returns a FloatBuffer object that contains the matrix data.
	 * @param columnMajor if true, this buffer should be filled with column major data, otherwise it will be filled row major.
	...
	/**
	 * <code>fillFloatBuffer</code> fills a FloatBuffer object with the matrix data.
	 * @param fb the buffer to fill, starting at current position. Must have room for 16 more floats.
	 * @param columnMajor if true, this buffer should be filled with column major data, otherwise it will be filled row major.
	...
	/**
	 * <code>fromAngleAxis</code> sets this matrix4f to the values specified by an angle and an axis of rotation. This method creates an object, so use fromAngleNormalAxis if your axis is already normalized.
	...
	/**
	 * <code>mult</code> multiplies this matrix with another matrix. The result matrix will then be returned. This matrix will be on the left hand side, while the parameter matrix will be on the right.
	 ...
	 * <code>mult</code> multiplies this matrix with another matrix. The result matrix will then be returned. This matrix will be on the left hand side, while the parameter matrix will be on the right.
	 * @param in2 the matrix to multiply this matrix by.
	 * @param store where to store the result. It is safe for in2 and store to be the same object.
	 * @return the resultant matrix
	 */
	...
	/**
	 * <code>mult</code> multiplies this matrix with another matrix. The results are stored internally and a handle to this matrix will then be returned. This matrix will be on the left hand side, while the parameter matrix will be on the right.
	 * @param in2 the matrix to multiply this matrix by.
	 * @return the resultant matrix
	 */
	...
	/**
	 * <code>angleRotation</code> sets this matrix to that of a rotation about three axes (x, y, z). Where each axis has a specified rotation in degrees. These rotations are expressed in a single <code>Vector3f</code> object.
	...
	/**
	 * <code>toString</code> returns the string representation of this object. It is in a format of a 4x4 matrix. For example, an identity matrix would be represented by the following string. com.jme.math.Matrix3f <br>
	 * [<br>
	 * 1.0 0.0 0.0 0.0 <br>
	 * 0.0 1.0 0.0 0.0 <br>
	 * 0.0 0.0 1.0 0.0 <br>
