...
public class EventScheduler implements Runnable
{
	...
	{
		final ScheduledFuture<?> future = ThreadPoolManager.getInstance().scheduleAtFixedRate(new EventScheduleWrapper(event), delay, period);
		return future;
	}
	...
	void waitForExecution(Event event)
	{
		...
		{
			log.warn("An event of class " + event.getClass().getName() + " have runned over " + TIMEOUT + " minutes.\nPlease check whether finish() is called!");
			startTime = 0;
			ThreadPoolManager.getInstance().schedule(this, event.getCooldown());
		}
		else
